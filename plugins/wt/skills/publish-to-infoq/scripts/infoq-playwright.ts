...
async function waitForPageReady(page: Page, timeoutMs = 20000): Promise<void> {
	...
	console.log(
		"[infoq-pw] Editor not detected via standard selectors, continuing anyway...",
	);
}
...
async function checkLoginStatus(page: Page): Promise<boolean> {
	...
	if (
		...
		currentUrl.includes("/article/create") ||
	...
	try {
		...
		const accountArea = page.locator(
			".header .account, .nav-right .account, header .user-info, .header .avatar",
		);

		// Presence of avatar or user-specific elements in the account area
		const hasUserIndicator = await accountArea
			.locator(
				'.user-avatar, [class*="avatar"], .user-info, [class*="user"], img',
			)
			.first()
			.isVisible();
		if (hasUserIndicator) {
			// One more check: make sure "登录" text is NOT the only thing in account area
			const loginText = await accountArea
				.locator(':text("登录")')
				.first()
				.isVisible();
			if (!loginText) {
		...
		const hasLoginButton = await accountArea
			.locator(':text("登录"), :text("注册"), :text("Sign in")')
			.first()
			.isVisible();
		if (hasLoginButton) {
			return false;
		...
		if (isAccountAreaPresent) {
			...
			const writeBtnPresent = await page
				.locator(':text("写点什么")')
				.first()
				.isVisible();
			if (writeBtnPresent) {
...
async function fillSubtitle(page: Page, subtitle: string): Promise<void> {
	...
	const result = await trySelectors(page, INFOQ_SELECTORS.subtitleInput, {
		timeout: 3000,
		visible: false,
	});
	if (!result.found || !result.locator) {
...
async function fillContent(page: Page, content: string): Promise<void> {
	...
	const result = await trySelectors(page, INFOQ_SELECTORS.contentEditor, {
		timeout: 5000,
		visible: true,
	});
	if (!result.found || !result.locator) {
	...
	try {
		console.log(
			"[infoq-pw] Attempting to fill content via HTML clipboard pasting...",
		);
		...
		console.log(
			"[infoq-pw] Clipboard paste produced minimal content, trying fallbacks...",
		);
	} catch (e) {
...
async function setCategory(page: Page, category?: string): Promise<void> {
	...
	const result = await trySelectors(page, INFOQ_SELECTORS.categorySelect, {
		timeout: 2000,
		visible: false,
	});
	if (!result.found || !result.locator) {
		console.log(
			"[infoq-pw] Category selector not found (optional, skipping...)",
		);
		return;
	}
...
async function addTags(page: Page, tags: string[]): Promise<void> {
	...
	const result = await trySelectors(page, INFOQ_SELECTORS.tagsInput, {
		timeout: 2000,
		visible: false,
	});
	if (!result.found || !result.locator) {
...
export async function publishToInfoQ(options: PublishOptions): Promise<string> {
	...
	try {
		...
		if (!isLoggedIn) {
			console.log(
				"[infoq-pw] Not logged in. Please log in to your InfoQ account.",
			);
			console.log(
				"[infoq-pw] Waiting for login (manual intervention required)...",
			);
		...
		if (
			...
			console.log(
				'[infoq-pw] Not on editor page, attempting to click "Write" button...',
			);

			const writeBtnResult = await trySelectors(
				page,
				[
					".write-btn",
					':text("写点什么")',
			...
			if (writeBtnResult.found && writeBtnResult.locator) {
				...
				if (pages.length > 1) {
					console.log(
						"[infoq-pw] Multiple pages detected, switching to newest...",
					);
				...
				console.log(
					'[infoq-pw] "Write" button not found, trying direct navigation to possible URLs...',
				);
				const possibleUrls = [
				...
				for (const url of possibleUrls) {
					...
					if (page.url().includes("write") || page.url().includes("create")) {
						console.log(
							`[infoq-pw] Successfully reached editor: ${page.url()}`,
						);
						break;
					}
				}
		...
		console.log(
			`[infoq-pw] Status: ${asDraft ? "draft" : "submitted for review"}`,
		);
