...
async function waitForPageReady(page: Page, timeoutMs = 20000): Promise<void> {
    ...
    while (Date.now() - start < timeoutMs) {
        try {
            ...
            const editorResult = await trySelectors(page, INFOQ_SELECTORS.contentEditor, {
                timeout: 2000,
                visible: true,
            });
...
async function checkLoginStatus(page: Page): Promise<boolean> {
    ...
    try {
        ...
        const hasUserIndicator = await accountArea
            .locator('.user-avatar, [class*="avatar"], .user-info, [class*="user"], img')
            .first()
            .isVisible();
        if (hasUserIndicator) {
    ...
    try {
        ...
        if (!currentUrl.includes('/write') && !currentUrl.includes('/article/create')) {
            ...
            const writeBtnResult = await trySelectors(
                page,
                ['.write-btn', ':text("写点什么")', ':text("立即创作")', '.account .writing', '.account .btn-write'],
