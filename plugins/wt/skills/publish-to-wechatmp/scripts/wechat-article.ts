...
// Import CDP-related functions from cdp.ts
import { launchChrome, getPageSession, waitForNewTab, evaluate, clickElement, typeText, pasteFromClipboard, type ChromeSession, CdpConnection } from './cdp.js';
...
// Local helpers
function sleepCompat(ms: number): Promise<void> {
  return sleep(ms);
}
...
export async function postArticle(options: ArticleOptions): Promise<void> {
  const { title, content, htmlFile, markdownFile, theme, author, summary, images = [], submit = false, profileDir = getDefaultProfileDir() } = options;
...
async function main(): Promise<void> {
  ...
  for (let i = 0; i < args.length; i++) {
    ...
    else if (arg === '--submit') submit = true; // Explicit flag overrides config
    else if (arg === '--no-submit') submit = false; // Explicit flag overrides config
  ...
  if (theme === undefined) {
    theme = getDefaultThemePreference(); // Default from config
  }
