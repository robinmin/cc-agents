...
async function clickMenuItem(page: Page, text: string): Promise<void> {
	...
	for (let i = 0; (await menuItems.nth(i).count()) > 0; i++) {
		...
		const itemTextAlt = await menuItems
			.nth(i)
			.textContent()
			.catch(() => "");
...
async function waitForEditor(page: Page, timeoutMs = 30000): Promise<void> {
	...
	try {
		const result = await trySelectors(page, [WECHAT_SELECTORS.titleInput], {
			timeout: timeoutMs,
			visible: true,
		});
		if (!result.found) {
...
async function fillContent(page: Page, content: string): Promise<void> {
	...
	const result = await trySelectors(page, WECHAT_SELECTORS.contentEditor, {
		timeout: 10000,
		visible: false,
	});
	if (!result.found || !result.locator) {
		console.log(
			"[wechat-pw] Content editor not found, trying alternative approach...",
		);
...
async function submitOrSave(page: Page, submit: boolean): Promise<void> {
	if (submit) {
		...
		const result = await trySelectors(page, WECHAT_SELECTORS.submitButton, {
			timeout: 5000,
			visible: true,
		});
		if (result.found && result.locator) {
...
async function postToWeChatMP(options: WeChatPlaywrightOptions): Promise<void> {
	...
	if (options.markdownFile) {
		...
		console.log(
			`[wechat-pw] Parsed markdown: title="${meta.title}", content=${meta.content.length} chars`,
		);
	}
	...
	if (options.imagesDir) {
		...
		if (!fs.existsSync(absDir))
			throw new Error(`Images directory not found: ${absDir}`);
	...
	if (!content)
		throw new Error("Content is required (use --content or --markdown)");
	if (images.length === 0)
		throw new Error(
			"At least one image is required (use --image or --imagesDir)",
		);
	...
	if (content.length > 1000) {
		...
		console.log(
			`[wechat-pw] Content compressed: ${original} → ${content.length} chars`,
		);
	}
	...
	try {
		...
		console.error(
			`[wechat-pw] Error: ${error instanceof Error ? error.message : String(error)}`,
		);
		throw error;
	} finally {
