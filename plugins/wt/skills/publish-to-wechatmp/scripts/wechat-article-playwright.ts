...
import { expandTilde, getWtConfig } from "@wt/web-automation/config";
...
const WECHAT_SELECTORS = {
	...
	submitButton:
		'#js_submit button, button.js_submit, .weui-desktop-btn_primary:has-text("发布"), .weui-desktop-btn_primary:has-text("发表"), button[type="submit"]',
	...
	coverNextBtn:
		'.js_btn_next, .btn-next, button:has-text("下一步"), button:has-text("下一步")',
	coverConfirmBtn:
		'.js_btn_confirm, .btn-confirm, button:has-text("确认"), button:has-text("确认")',
	// Original setting
	originalCheckbox:
		'.js_original_radio, [class*="original"]:radio, input[name*="original"], label:has-text("原创"), [class*="original"]:label, .original_checkbox',
	originalConfirmBtn:
		'.js_btn_original, button:has-text("确认"), button:has-text("确认")',
	// Reward
	rewardBtn:
		'.js_reward_btn, [class*="reward"]:button, button:has-text("赞赏")',
	rewardConfirmBtn:
		'.js_btn_reward, button:has-text("确认"), button:has-text("确认")',
	// Collection
	collectionBtn:
		'.js_collection_btn, [class*="collection"]:button, button:has-text("合集"), button:has-text("未添加")',
	collectionInput:
		'.js_collection_input, input[name*="collection"], input[placeholder*="合集"]',
	collectionConfirmBtn:
		'.js_btn_collection, button:has-text("确认"), button:has-text("确认")',
	// Publish workflow
	saveDraftBtn:
		'.js_save_draft, button:has-text("保存为草稿"), button:has-text("保存草稿")',
...
async function _styleCodeBlocks(_page: Page): Promise<void> {
...
async function waitForArticleEditor(
	...
	try {
		await page
			.locator(WECHAT_SELECTORS.titleInput)
...
async function fillAuthor(page: Page, author: string): Promise<void> {
	...
	// Debug screenshot
	await page
		.screenshot({ path: "/tmp/wechat_author_before.png" })
		.catch(() => {});
	...
	console.log(
		`[wechat-article-pw] Found ${placeholderCount} input(s) with "作者" placeholder`,
	);
	...
	for (let i = 0; i < placeholderCount; i++) {
		...
		try {
			...
			if (isVisible) {
				...
				console.log(
					`[wechat-article-pw] Author filled via placeholder (index ${i})`,
				);
				await page
					.screenshot({ path: "/tmp/wechat_author_after.png" })
					.catch(() => {});
				return;
			}
		} catch {}
			...
			if (isVisible) {
				...
				if (inputId) {
					...
					if ((await input.count()) > 0) {
						await input.fill(author);
						console.log(
							`[wechat-article-pw] Author filled via label for (index ${i})`,
						);
						await page
							.screenshot({ path: "/tmp/wechat_author_after.png" })
							.catch(() => {});
				...
				if ((await parentInput.count()) > 0) {
					...
					console.log(
						`[wechat-article-pw] Author filled via parent input (index ${i})`,
					);
					await page
						.screenshot({ path: "/tmp/wechat_author_after.png" })
						.catch(() => {});
	...
	for (let i = 0; i < inputCount; i++) {
		...
		try {
			...
			if (
				placeholder &&
				(placeholder.includes("作者") || placeholder.includes("请输入"))
			) {
				await input.fill(author);
				console.log(
					`[wechat-article-pw] Author filled via text input (index ${i})`,
				);
				await page
					.screenshot({ path: "/tmp/wechat_author_after.png" })
					.catch(() => {});
...
async function fillAuthorFinal(page: Page, author: string): Promise<void> {
	...
	await page
		.screenshot({ path: "/tmp/wechat_author_final_before.png" })
		.catch(() => {});
	...
	for (let i = 0; i < inputCount; i++) {
		...
		try {
			...
			const isAuthorField =
				(placeholder &&
					(placeholder.includes("作者") || placeholder.includes("author"))) ||
			...
			if (isAuthorField) {
				...
				await page
					.screenshot({ path: "/tmp/wechat_author_final_after.png" })
					.catch(() => {});
				return;
			}
		} catch {}
	...
	for (let i = 0; i < labelCount; i++) {
		...
		try {
			...
			const siblingInput = parent
				.locator('input[type="text"], input:not([type])')
				.first();
			if ((await siblingInput.count()) > 0) {
				await siblingInput.fill(author);
				console.log(
					`[wechat-article-pw] Author filled via sibling (index ${i})`,
				);
				await page.waitForTimeout(500);
				await page
					.screenshot({ path: "/tmp/wechat_author_final_after.png" })
...
async function uploadCoverImageWorkflow(
	page: Page,
	coverPath: string,
): Promise<void> {
	...
	await page
		.screenshot({ path: "/tmp/wechat_cover_1_before.png" })
		.catch(() => {});
	...
	await page
		.screenshot({ path: "/tmp/wechat_cover_2_after_click.png" })
		.catch(() => {});
	...
	await page
		.screenshot({ path: "/tmp/wechat_cover_3_after_upload.png" })
		.catch(() => {});
	...
	const coverItems = page.locator(
		'.imgpicker-item, .cover-item, [class*="item"]',
	);
	...
	await page
		.screenshot({ path: "/tmp/wechat_cover_4_after_select.png" })
		.catch(() => {});

	// Step 4: Look for any visible "确定" or "确认" button
	console.log(
		"[wechat-article-pw] Step 4: Looking for confirmation buttons...",
	);

	// Try all possible button selectors
	const allButtons = page.locator(
		'button, .weui-desktop-btn_primary, a[class*="btn"]',
	);
	...
	for (const selector of confirmSelectors) {
		...
		if ((await btn.count()) > 0) {
			try {
				...
				if (isVisible) {
					const text =
						(await btn.textContent({ timeout: 500 }).catch(() => "")) || "";
					await btn.click();
					console.log(
						`[wechat-article-pw] Clicked: "${text.trim()}" via ${selector}`,
					);
					await page.waitForTimeout(2000);
					break;
				}
	...
	await page
		.screenshot({ path: "/tmp/wechat_cover_5_final.png" })
		.catch(() => {});
...
async function setOriginal(page: Page, author: string): Promise<void> {
	...
	// Debug screenshot
	await page
		.screenshot({ path: "/tmp/wechat_original_1_before.png" })
		.catch(() => {});
	...
	for (const valueText of valueTexts) {
		...
		console.log(
			`[wechat-article-pw] Found ${count} element(s) with "${valueText}" text`,
		);
		...
		for (let i = 0; i < count && !clicked; i++) {
			...
			try {
				...
				if (isVisible) {
					...
					const nearOriginal = await el
						.locator('xpath=./preceding::*[contains(text(), "原创")]')
						.count();
					const parentHasOriginal = await el
						.locator('xpath=./ancestor::*[contains(text(), "原创")]')
						.count();

					if (nearOriginal > 0 || parentHasOriginal > 0) {
						console.log(
							`[wechat-article-pw] Found 原创 value "${valueText}" (verified nearby)`,
						);
	...
	if (!clicked) {
		...
		console.log(
			`[wechat-article-pw] Found ${labelCount} element(s) with "原创" text (fallback)`,
		);
			...
			try {
				...
				if (isVisible) {
					...
					for (let sibIdx = 1; sibIdx <= 5; sibIdx++) {
						const nextSibling = label.locator(
							`xpath=./following-sibling::*[${sibIdx}]`,
						);
						const siblingTextRaw = await nextSibling
							.textContent({ timeout: 500 })
							.catch(() => "");
						const siblingText =
							siblingTextRaw?.replace(/\s+/g, " ").trim() || "";
						console.log(
							`[wechat-article-pw] Sibling ${sibIdx} text after 原创: "${siblingText}"`,
						);

						if (
							siblingText.includes("未声明") ||
							siblingText.includes("已声明")
						) {
							await nextSibling.evaluate((e: Element) =>
								(e as HTMLElement).click(),
							);
							console.log(
								`[wechat-article-pw] Clicked 原创 value: "${siblingText}"`,
							);
							clicked = true;
							break;
						}
	...
	console.log(
		"[wechat-article-pw] Original dialog opened, completing declaration...",
	);
	await page.waitForTimeout(1500);

	// Debug screenshot after dialog opens
	await page
		.screenshot({ path: "/tmp/wechat_original_2_dialog.png" })
		.catch(() => {});
	...
	for (const selector of authorInputSelectors) {
		...
		try {
			...
			if (count > 0) {
				const isVisible = await authorInput
					.first()
					.isVisible({ timeout: 500 })
					.catch(() => false);
				if (isVisible) {
					const currentValue = await authorInput
						.first()
						.inputValue()
						.catch(() => "");
					if (!currentValue || currentValue.trim() === "") {
						...
						console.log(
							`[wechat-article-pw] Author filled in popup: "${author}"`,
						);
					} else {
						console.log(
							`[wechat-article-pw] Author already filled in popup: "${currentValue}"`,
						);
					}
					break;
				}
	...
	const agreementLabel = page.locator(
		'label:has-text("我已阅读并同意"), label:has-text("同意《微信公众平台原创声明及相关功能使用协议》")',
	);
	...
	for (let i = 0; i < labelCount && !agreementChecked; i++) {
		...
		try {
			...
			if (isVisible) {
				...
				console.log(
					`[wechat-article-pw] Found ${checkboxCount} checkbox(es) in agreement label`,
				);
				...
				if (checkboxCount > 0) {
					...
					if (!isChecked) {
						...
						console.log(
							`[wechat-article-pw] Agreement checkbox already checked`,
						);
					}
	...
	if (!agreementChecked) {
		...
		const dialogCheckboxes = page.locator(
			'.weui-desktop-dialog input[type="checkbox"]:not(:checked), .weui-desktop-modal input[type="checkbox"]:not(:checked), [class*="dialog"] input[type="checkbox"]:not(:checked)',
		);
		const dialogCheckboxCount = await dialogCheckboxes.count();
		console.log(
			`[wechat-article-pw] Found ${dialogCheckboxCount} unchecked checkbox(es) in dialog`,
		);
		...
		if (dialogCheckboxCount > 0) {
			...
			console.log(
				`[wechat-article-pw] Checked first unchecked checkbox in dialog`,
			);
			agreementChecked = true;
		}
	}
	...
	console.log(
		'[wechat-article-pw] ⚠️  Original dialog opened - please manually click "确定" to confirm',
	);
...
async function setOriginalSource(page: Page): Promise<void> {
	...
	// Debug screenshot
	await page
		.screenshot({ path: "/tmp/wechat_source_1_before.png" })
		.catch(() => {});
	...
	console.log(
		`[wechat-article-pw] Found ${labelCount} element(s) with "创作来源" text`,
	);
	...
	for (let i = 0; i < labelCount && !clicked; i++) {
		...
		try {
			...
			if (isVisible) {
				...
				const siblingText = await nextSibling
					.textContent({ timeout: 500 })
					.catch(() => "");
				console.log(
					`[wechat-article-pw] Sibling text after 创作来源 (${i}): "${siblingText}"`,
				);

				if (
					siblingText?.includes("未添加") ||
					siblingText?.includes("已添加")
				) {
					...
					console.log(
						`[wechat-article-pw] Clicked 创作来源 value: "${siblingText}"`,
					);
					clicked = true;
					break;
				}
				...
				if (parentText) {
					const valueInParent = parent
						.locator("text=未添加, text=已添加")
						.first();
					if ((await valueInParent.count()) > 0) {
						await valueInParent.evaluate((e: Element) =>
							(e as HTMLElement).click(),
						);
	...
	await page.waitForTimeout(2000);
	await page
		.screenshot({ path: "/tmp/wechat_source_2_after_notadd.png" })
		.catch(() => {});
	...
	console.log(
		`[wechat-article-pw] Found ${declCount} element(s) with "无需声明"`,
	);
		...
		try {
			...
			if (isVisible) {
				...
				if ((await checkboxInParent.count()) > 0) {
					await checkboxInParent
						.first()
						.evaluate((e: Element) => (e as HTMLElement).click());
	...
	await page
		.screenshot({ path: "/tmp/wechat_source_3_after_select.png" })
		.catch(() => {});
	...
	const confirmBtn = page
		.locator(
			'.weui-desktop-modal button:has-text("确认"), button:has-text("确认")',
		)
		.first();
	if ((await confirmBtn.count()) > 0) {
		try {
			...
			await confirmBtn.waitFor({ state: "hidden", timeout: 3000 }).catch(() => {
				console.log(
					"[wechat-article-pw] Note: Source dialog may still be visible",
				);
			});
		} catch {
	...
	await page
		.screenshot({ path: "/tmp/wechat_source_4_final.png" })
		.catch(() => {});
...
async function enableReward(page: Page): Promise<void> {
	...
	// Debug screenshot
	await page
		.screenshot({ path: "/tmp/wechat_reward_1_before.png" })
		.catch(() => {});
		...
		for (let i = 0; i < count && !clicked; i++) {
			...
			try {
				...
				if (isVisible) {
					...
					const nearReward = await el
						.locator('xpath=./preceding::*[contains(text(), "赞赏")]')
						.count();
					const parentHasReward = await el
						.locator('xpath=./ancestor::*[contains(text(), "赞赏")]')
						.count();

					if (nearReward > 0 || parentHasReward > 0) {
						console.log(
							`[wechat-article-pw] Found 赞赏 value "${valueText}" (verified nearby)`,
						);
						await el.evaluate((e: Element) => (e as HTMLElement).click());
						clicked = true;
						break;
	...
	if (!clicked) {
		...
		console.log(
			`[wechat-article-pw] Found ${labelCount} element(s) with "赞赏" text (fallback)`,
		);
				...
				if (isVisible) {
					...
					for (let sibIdx = 1; sibIdx <= 5; sibIdx++) {
						...
						const siblingText =
							siblingTextRaw?.replace(/\s+/g, " ").trim() || "";
						console.log(
							`[wechat-article-pw] Sibling ${sibIdx} text after 赞赏: "${siblingText}"`,
						);

						if (
							siblingText.includes("未开启") ||
							siblingText.includes("已开启")
						) {
							await nextSibling.evaluate((e: Element) =>
								(e as HTMLElement).click(),
							);
							console.log(
								`[wechat-article-pw] Clicked 赞赏 value: "${siblingText}"`,
							);
							clicked = true;
							break;
						}
	...
	await page.waitForTimeout(500);
	await page
		.screenshot({ path: "/tmp/wechat_reward_2_after.png" })
		.catch(() => {});
	...
	// Debug screenshot after dialog opens
	await page
		.screenshot({ path: "/tmp/wechat_reward_2_dialog.png" })
		.catch(() => {});
		...
		for (let i = 0; i < count && !enabled; i++) {
			...
			try {
				...
				if (isVisible) {
					...
					const parent = el.locator("..").first();
					const checkboxInParent = parent.locator(
					...
					if ((await checkboxInParent.count()) > 0) {
						await checkboxInParent
							...
							.evaluate((e: Element) => (e as HTMLElement).click());
						console.log(
							`[wechat-article-pw] Enabled reward via checkbox near "${text}"`,
						);
						enabled = true;
						break;
					}
	...
	if (!enabled) {
		const toggles = page.locator(
			'.weui-desktop-switch, .weui-switch, [class*="switch"], [class*="toggle"]',
		);
			...
			try {
				...
				try {
					...
					try {
						...
						if (handle) {
							await page.evaluate((el: Element) => el.click(), handle);
						}
					}
				}
				console.log(
					`[wechat-article-pw] Reward dialog confirmed via: ${selector}`,
				);
				confirmClicked = true;
				break;
			} catch {}
	...
	if (!confirmClicked) {
		console.log(
			"[wechat-article-pw] Warning: Could not click reward confirm button",
		);
	}
	...
	for (let i = 0; i < 5; i++) {
		...
		console.log(
			`[wechat-article-pw] Waiting for reward value to save... (${i + 1}/5)`,
		);
	}

	if (!rewardSaved) {
		console.log(
			"[wechat-article-pw] Warning: Reward value may not have been saved",
		);
	}
	...
	await page.waitForTimeout(500);
	await page
		.screenshot({ path: "/tmp/wechat_reward_3_after.png" })
		.catch(() => {});
...
async function setCollection(page: Page, collection: string): Promise<void> {
	...
	// Debug screenshot
	await page
		.screenshot({ path: "/tmp/wechat_collection_1_before.png" })
		.catch(() => {});
	...
	console.log(
		`[wechat-article-pw] Found ${labelCount} element(s) with "合集" text`,
	);
	...
	for (let i = 0; i < labelCount && !clicked; i++) {
		...
		try {
			...
			if (isVisible) {
				...
				console.log(
					`[wechat-article-pw] Sibling text after 合集 (${i}): "${siblingText}"`,
				);
				...
				if (
					...
					console.log(
						`[wechat-article-pw] Clicked 合集 value: "${siblingText}"`,
					);
					clicked = true;
					break;
				}
				...
				// Also try the parent
				const parent = label.locator("..").first();
				const parentTextRaw = await parent
					.textContent({ timeout: 500 })
					.catch(() => "");
	...
	await page.waitForTimeout(500);
	await page
		.screenshot({ path: "/tmp/wechat_collection_2_dialog.png" })
		.catch(() => {});
			...
			for (let i = 0; i < count; i++) {
				...
				try {
					...
					if (isVisible) {
						...
						console.log(
							`[wechat-article-pw] Collection name entered via: ${selector}`,
						);
						break;
					}
				} catch {}
	...
	await page
		.screenshot({ path: "/tmp/wechat_collection_3_after.png" })
		.catch(() => {});

	// Leave dialog open for user to manually click confirm
	console.log(
		'[wechat-article-pw] ⚠️  Collection dialog opened - please manually click "确认" to confirm',
	);
	await page.waitForTimeout(1000);
}
...
async function insertHtmlContent(page: Page, htmlPath: string): Promise<void> {
	...
	console.log(
		`[wechat-article-pw] HTML content length: ${htmlContent.length} chars`,
	);
...
async function insertContentImages(
	...
	console.log(
		`[wechat-article-pw] Inserting ${contentImages.length} content image(s)...`,
	);
	...
	for (let i = 0; i < contentImages.length; i++) {
		...
		if (!img) continue;
		console.log(
			`[wechat-article-pw] [${i + 1}/${contentImages.length}] ${img.placeholder}: ${path.basename(img.localPath)}`,
		);
		console.log(
			`[wechat-article-pw] Note: Content image replacement requires copy-to-clipboard script`,
		);
		await page.waitForTimeout(500);
	}
	console.log(
		"[wechat-article-pw] Content images logged (manual insertion required)",
	);
}
...
async function saveDraft(page: Page): Promise<void> {
	...
	try {
		...
		console.log(
			"[wechat-article-pw] Save draft button not found, trying submit button...",
		);
...
async function publishArticle(page: Page): Promise<void> {
	...
	if (!published) {
		console.log(
			"[wechat-article-pw] Publish button not found, article saved as draft",
		);
		return;
	}
	...
	const qrDialog = page.locator(
		'.weui-desktop-modal, [class*="qrcode"], [class*="QR"], .qr_code_dialog',
	);
	...
	const noSourceBtn = page
		.locator(
			'button:has-text("无需声明并发表"), .weui-desktop-btn_primary:has-text("无需声明")',
		)
		.first();
...
async function postArticle(options: ArticleOptions): Promise<void> {
	...
	const effectiveCollection =
		optionCollection || getCollectionFromConfig() || "";
	...
	if (options.markdownFile) {
		...
		effectiveCoverImage =
			effectiveCoverImage ||
			(parsed.coverImage
				? path.isAbsolute(parsed.coverImage)
					? parsed.coverImage
					: path.resolve(path.dirname(absPath), parsed.coverImage)
				: "");
		...
		console.log(
			`[wechat-article-pw] Cover image: ${effectiveCoverImage || "(empty)"}`,
		);
		console.log(
			`[wechat-article-pw] Collection: ${effectiveCollection || "(empty)"}`,
		);
	...
	try {
		...
		if (submit) {
			...
			console.log(
				'[wechat-article-pw] Add --collection "合集名" to set collection',
			);
		}
		...
		await page
			.screenshot({ path: "/tmp/wechat_article_final.png", fullPage: true })
			.catch(() => {});
		console.log(
			"[wechat-article-pw] Final screenshot saved: /tmp/wechat_article_final.png",
		);
		...
		console.error(
			`[wechat-article-pw] Error: ${error instanceof Error ? error.message : String(error)}`,
		);
		throw error;
	} finally {
