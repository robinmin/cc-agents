 ...
 */
export async function loadAndRegisterLanguage(
	language: string,
	hljs: HLJSApi,
): Promise<void> {
	// 如果已经注册，直接返回
...
export function highlightAndFormatCode(
	...
	if (showLineNumber) {
		...
		const lineNumbersHtml = highlightedLines
			.map(
				(_, idx) =>
					`<section style="padding:0 10px 0 0;line-height:1.75">${idx + 1}</section>`,
			)
			.join(``);
...
export function highlightCodeBlock(
	...
	const showLineNumber =
		codeBlock.getAttribute(`data-show-line-number`) === `true`;
 ...
 */
export function highlightPendingBlocks(
	hljs: HLJSApi,
	container: Document | Element = document,
): void {
	const pendingBlocks = container.querySelectorAll(
		`code[data-language-pending]`,
	);
