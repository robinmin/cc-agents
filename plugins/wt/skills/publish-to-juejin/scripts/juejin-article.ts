...
async function waitForPageReady(
	...
	console.log(
		"[juejin] Editor not detected via standard selectors, continuing anyway...",
	);
}
	...
	while (Date.now() - start < timeoutMs) {
		try {
			const selector = await findElement(
				session,
				JUEJIN_SELECTORS.subtitleInput,
				1000,
			);
...
async function setCategory(
	...
	try {
		const selector = await findElement(
			session,
			JUEJIN_SELECTORS.categorySelect,
			2000,
		);
...
async function submitArticle(
	...
	const buttonSelectors = asDraft
		? JUEJIN_SELECTORS.draftButton
		: JUEJIN_SELECTORS.publishButton;
...
export async function publishToJuejin(
	...
	try {
		...
		if (!isLoggedIn) {
			...
			console.log(
				"[juejin] Waiting for login (manual intervention required)...",
			);
