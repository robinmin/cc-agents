...
async function waitForPageReady(page: Page, timeoutMs = 20000): Promise<void> {
    ...
    while (Date.now() - start < timeoutMs) {
        try {
            ...
            const editorResult = await trySelectors(page, JUEJIN_SELECTORS.contentEditor, {
                timeout: 2000,
                visible: true,
            });
...
async function checkLoginStatus(page: Page): Promise<boolean> {
    ...
    try {
        const isLoggedIn =
            (await page.locator('.user-avatar, .logout-button, [class*="user"], .avatar').first().count()) > 0;
        return isLoggedIn;
    } catch {
        return false;
    }
...
async function submitArticle(page: Page, asDraft: boolean, article: ParsedArticle): Promise<string> {
    ...
    try {
        ...
        const finalResult = await trySelectors(page, JUEJIN_SELECTORS.confirmPublishButton, {
            timeout: 3000,
            visible: true,
        });
