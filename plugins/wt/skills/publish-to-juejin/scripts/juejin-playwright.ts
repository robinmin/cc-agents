...
async function waitForPageReady(page: Page, timeoutMs = 20000): Promise<void> {
	...
	while (Date.now() - start < timeoutMs) {
		try {
			...
			const editorResult = await trySelectors(
				page,
				JUEJIN_SELECTORS.contentEditor,
	...
	console.log(
		"[juejin-pw] Editor not detected via standard selectors, continuing anyway...",
	);
}
...
async function checkLoginStatus(page: Page): Promise<boolean> {
	...
	if (
		...
		currentUrl.includes("/markdown-editor")
	) {
		return true;
	}
	...
	try {
		const isLoggedIn =
			(await page
				.locator('.user-avatar, .logout-button, [class*="user"], .avatar')
				.first()
				.count()) > 0;
		return isLoggedIn;
	} catch {
...
async function fillTitle(page: Page, title: string): Promise<void> {
	...
	const result = await trySelectors(page, JUEJIN_SELECTORS.titleInput, {
		timeout: 5000,
		visible: true,
	});
...
async function fillSubtitle(page: Page, subtitle: string): Promise<void> {
	...
	const result = await trySelectors(page, JUEJIN_SELECTORS.subtitleInput, {
		timeout: 3000,
		visible: false,
	});
	if (!result.found || !result.locator) {
...
async function fillContent(page: Page, content: string): Promise<void> {
	...
	const result = await trySelectors(page, JUEJIN_SELECTORS.contentEditor, {
		timeout: 5000,
		visible: true,
	});
	if (!result.found || !result.locator) {
	...
	try {
		...
		if (isCodeMirror) {
			...
			await page.evaluate((text) => {
				...
				const win = window as unknown as { editor?: CodeMirrorEditor };
...
async function setCategory(page: Page, category?: string): Promise<void> {
	...
	try {
		const item = page
			.locator(`.category-list .item:has-text("${category}")`)
			.first();
		...
		console.log(
			`[juejin-pw] Failed to set category: ${category}. Error: ${err instanceof Error ? err.message : String(err)}`,
		);
	}
}
...
async function addTags(page: Page, tags: string[]): Promise<void> {
	...
	const inputResult = await trySelectors(page, [JUEJIN_SELECTORS.tagsInput], {
		timeout: 2000,
		visible: true,
	});
...
async function submitArticle(
	...
	try {
		...
		const result = await trySelectors(page, JUEJIN_SELECTORS.publishButton, {
			timeout: 3000,
			visible: true,
		});
		if (!result.found || !result.locator) {
		...
		const finalResult = await trySelectors(
			page,
			JUEJIN_SELECTORS.confirmPublishButton,
		...
		console.log(
			`[juejin-pw] Publish failed: ${error instanceof Error ? error.message : String(error)}`,
		);
		throw error;
	}
}
 ...
 */
export async function publishToJuejin(
	...
	try {
		...
		if (!isLoggedIn) {
			console.log(
				"[juejin-pw] Not logged in. Please log in to your Juejin account.",
			);
			console.log(
				"[juejin-pw] Waiting for login (manual intervention required)...",
			);
