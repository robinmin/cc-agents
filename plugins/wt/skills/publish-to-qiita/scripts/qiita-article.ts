...
export async function installQiitaCli(projectPath: string): Promise<void> {
	...
	if (fs.existsSync(packageJsonPath)) {
		try {
			...
			if (
				pkg.dependencies?.["@qiita/qiita-cli"] ||
				pkg.devDependencies?.["@qiita/qiita-cli"]
			) {
	...
	await runCommand(projectPath, "npm", [
		"install",
		"@qiita/qiita-cli",
		"--save-dev",
	]);
 ...
 */
export async function publishToQiitaCli(
	...
	if (options.markdownFile) {
		...
		throw new Error(
			"Error: --markdown is required (or use --title with --content and --tags)",
		);
	}
	...
	if (options.organization !== undefined)
		article.organization_url_name = options.organization;
