...
const XHS_SELECTORS = {
    ...
    // Publish button
    publishButton: [':text("一键排版")', ':text("下一步")', '.publish-btn', 'button:text("下一步")'],
...
async function waitForPageReady(page: Page, timeoutMs = 20000): Promise<void> {
    ...
    while (Date.now() - start < timeoutMs) {
        try {
            ...
            const editorResult = await trySelectors(page, XHS_SELECTORS.contentEditor, {
                timeout: 2000,
                visible: true,
            });
    ...
    try {
        ...
        try {
            ...
            const titleCandidate = await page
                .locator('[contenteditable="true"], .input-title, [placeholder*="标题"]')
                .first();
