...
import { getFreePort, pwSleep as sleep } from '../../../scripts/web-automation/dist/playwright.js';
...
export async function publishArticle(options: ArticleOptions): Promise<void> {
    ...
    const connectWithRetry = async (): Promise<{
        ...
        for (let attempt = 1; attempt <= maxRetries; attempt++) {
            try {
                ...
                if (!pageTarget) {
                    const { targetId } = await cdpConn.send<{ targetId: string }>('Target.createTarget', {
                        url: X_ARTICLES_URL,
                    });
                    pageTarget = { targetId, url: X_ARTICLES_URL, type: 'page' };
                }

                const { sessionId: sessId } = await cdpConn.send<{ sessionId: string }>('Target.attachToTarget', {
    ...
    try {
        ...
        const needsNavigation =
            ...
            (!initialState.url.includes('x.com/compose/articles') && !initialState.url.includes('/articles/edit/'));
        ...
        if (currentUrl.includes('/preview')) {
            ...
            await cdp?.send('Page.navigate', { url: currentUrl.replace('/preview', '') }, { sessionId });
            await sleep(3000);
        }
