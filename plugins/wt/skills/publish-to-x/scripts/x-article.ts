...
const I18N_SELECTORS = {
	...
	saveButton: ['[data-testid="saveButton"]', 'button[aria-label*="Save" i]'],
	editorBody: [
		'.DraftEditor-editorContainer [contenteditable="true"]',
...
export async function publishArticle(options: ArticleOptions): Promise<void> {
	...
	if (verbose) {
		console.log(
			`[x-article] Content analysis: tables=${hasTables}, code blocks=${hasCodeBlocks}`,
		);
	}
	...
	if ((convertTables && hasTables) || (formatCodeBlocks && hasCodeBlocks)) {
		...
		console.log(
			`[x-article] Preprocessed: ${converted.tables.length} tables, ${converted.codeBlocks.length} code blocks`,
		);
	}
	...
	const chromePath =
		options.chromePath ?? findChromeExecutable(CHROME_CANDIDATES_BASIC);
	if (!chromePath) throw new Error("Chrome not found");

	let port = options.debugPort;
	let _chrome: ReturnType<typeof spawn> | null = null;
	let _weLaunchedChrome = false; // Track if we launched Chrome (for cleanup)
		...
		for (let attempt = 1; attempt <= maxRetries; attempt++) {
			try {
				if (options.useExisting) {
					...
					console.log(
						"[x-article] Looking for existing Chrome with remote debugging...",
					);
					...
					for (const tryPort of commonPorts) {
						...
						try {
							...
							const response = await fetch(
								`http://localhost:${tryPort}/json/version`,
								{
							...
							if (data.webSocketDebuggerUrl) {
								...
								connected = true;
								console.log(
									`[x-article] Found existing Chrome on port ${port}`,
								);
								break;
							}
						} catch {
				...
				if (verbose)
					console.log(`[x-article] Connecting to Chrome on port ${port}...`);
				const wsUrl = await waitForChromeDebugPort(port, 30_000);
				const cdpConn = await CdpConnection.connect(wsUrl, 30_000, {
					defaultTimeoutMs: 30_000,
				});

				// Get page target with retry
				const targets = await cdpConn.send<{
				...
				if (!pageTarget) {
					const { targetId } = await cdpConn.send<{ targetId: string }>(
						"Target.createTarget",
						{ url: X_ARTICLES_URL },
					);
				...
				const { sessionId: sessId } = await cdpConn.send<{ sessionId: string }>(
				...
				if (verbose)
					console.log(
						`[x-article] Connected successfully. Session ID: ${sessId}`,
					);
				...
				if (attempt < maxRetries) {
					const delay = 1000 * 1.5 ** (attempt - 1);
					console.log(
						`[x-article] Connection attempt ${attempt}/${maxRetries} failed, retrying in ${delay}ms...`,
					);
					await sleep(delay);
				} else {
					console.error(
						`[x-article] All ${maxRetries} connection attempts failed`,
					);
					throw lastError;
				}
			}
	...
	try {
		...
		console.log(
			"[x-article] Waiting for page to load (this may take 10-20 seconds)...",
		);
		...
		const waitForPageLoad = async (maxWaitMs = 45_000): Promise<boolean> => {
			return retry(
				async () => {
					...
					while (Date.now() - start < maxWaitMs) {
						const check = await cdp?.send<{
							result: { value: { bodyText: string; url: string } };
						}>(
							"Runtime.evaluate",
							{
								expression: `(() => {
              ...
              return { bodyText, url };
						...
						if (
							info.bodyText &&
							info.bodyText !== "no body text" &&
							info.bodyText.length > 10
						) {
							console.log(`[x-article] Page loaded! URL: ${info.url}`);
							console.log(
								`[x-article] Body text preview: ${info.bodyText.substring(0, 100)}...`,
							);
							return true;
						}
						...
						if (noProgressCount > maxNoProgress) {
							if (verbose) {
								console.log(
									`[x-article] No progress for 5 seconds, current body text length: ${currentBodyText.length}`,
								);
							}
							// If we're on a valid X URL with some content, consider it loaded
							if (info.url.includes("x.com") && currentBodyText.length > 5) {
								console.log(
									`[x-article] Page has minimal content but continuing...`,
								);
								return true;
							}
		...
		if (!pageLoaded) {
			...
			console.log(
				"[x-article] Page did not load properly. Current state:",
				state,
			);
			...
			if (!state.isLoaded && !state.isEditPage) {
				...
				console.log(
					"[x-article] Keeping browser open for 30 seconds for manual check...",
				);
				await sleep(30_000);
			}
		...
		const urlCheck = await cdp?.send<{ result: { value: string } }>(
			"Runtime.evaluate",
			{
		...
		if (currentUrl.includes("/preview")) {
			console.log(
				"[x-article] On preview page, navigating back to edit mode...",
			);
			await cdp?.send(
				"Page.navigate",
				{ url: currentUrl.replace("/preview", "") },
				{ sessionId },
			);
			await sleep(3000);
		}
		...
		if (currentUrl.includes("/articles/edit/")) {
			...
			const editorCheck = await cdp?.send<{
				result: { value: { hasEditor: boolean; hasWriteButton: boolean } };
			}>(
				"Runtime.evaluate",
				{
					expression: `(() => {
          ...
          return { hasEditor, hasWriteButton };
			...
			if (!hasEditor && !hasWriteButton) {
				console.log(
					"[x-article] On edit page without editor, navigating to compose page...",
				);
				await cdp?.send(
					"Page.navigate",
					{ url: X_ARTICLES_URL },
		...
		const elementScan = await cdp?.send<{ result: { value: string } }>(
			"Runtime.evaluate",
			{
				expression: `(() => {
        const result = {
		...
		for (const selector of I18N_SELECTORS.writeButton) {
			...
			if (await waitForElement(selector, 2000)) {
				console.log(
					`[x-article] Found Write button with selector: ${selector}`,
				);
		...
		if (!writeButtonClicked) {
			console.log(
				"[x-article] No Write button found, might already be on editor page",
			);
		}
		...
		if (!editorFound) {
			console.log(
				"[x-article] Title input not found, trying contenteditable fallback...",
			);
			const contentEditableFound = await waitForElement(
		...
		if (!editorFound) {
			...
			console.log(
				"[x-article] Keeping browser open for 60 seconds for manual check...",
			);
			await sleep(60_000);
		...
		await cdp.send<{ result: { value: boolean } }>(
			"Runtime.evaluate",
			{
				expression: `(() => {
        const editor = document.querySelector('.DraftEditor-editorContainer [contenteditable="true"]');
		...
		if (contentCheck.result.value > 50) {
			console.log(
				`[x-article] Content inserted successfully (${contentCheck.result.value} chars)`,
			);
		} else {
			console.log(
				"[x-article] Paste event may not have worked, trying insertHTML...",
			);
          ...
          return true;
			...
			if (check2.result.value > 50) {
				console.log(
					`[x-article] Content inserted via execCommand (${check2.result.value} chars)`,
				);
			} else {
				console.log(
		...
		if (parsed.contentImages.length > 0) {
			...
			for (let i = 0; i < sortedImages.length; i++) {
                ...
                return false;
						...
						const selectionCheck = await cdp?.send<{
							result: { value: string };
						}>(
							"Runtime.evaluate",
							{
				...
				console.log(
					`[x-article] Copying image: ${path.basename(img.localPath)}`,
				);
            ...
            return !editor.innerText.includes(${JSON.stringify(img.placeholder)});
				...
				if (!afterDelete.result.value) {
					console.warn(
						`[x-article] Placeholder may not have been deleted, trying again...`,
					);
            ...
            if (editor) editor.focus();
				...
				if (pasteFromClipboard("Google Chrome", 5, 1000)) {
					console.log(
						`[x-article] Image pasted: ${path.basename(img.localPath)}`,
					);
				} else {
