...
import { getFreePort, pwSleep as sleep } from "@wt/web-automation/playwright";
...
export async function postVideoToX(options: XVideoOptions): Promise<void> {
	...
	try {
		...
		if (!editorFound) {
			console.log(
				"[x-video] Editor not found. Please log in to X in the browser window.",
			);
		...
		if (videoReady) {
			...
			console.log(
				"[x-video] Video may still be processing. Please check browser window.",
			);
		}
		...
		if (submit) {
			...
			console.log(
				"[x-video] Post composed (preview mode). Add --submit to post.",
			);
			...
			setTimeout(() => {
				if (!chrome.killed)
					try {
						...
						console.debug("[x-video] SIGKILL error:", error);
					}
			}, 2_000).unref?.();
			try {
				chrome.kill("SIGTERM");
