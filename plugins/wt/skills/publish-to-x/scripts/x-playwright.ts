...
async function getChromiumPath(): Promise<string | undefined> {
	try {
		...
		console.log(
			"[x-playwright] Playwright Chromium not found, falling back to system Chrome",
		);
		return undefined;
	}
}
...
async function copyImage(imagePath: string): Promise<boolean> {
	try {
		const result = spawnSync("osascript", [
			"-e",
			`set the clipboard to (read "${imagePath}" as TIFF)`,
		]);
...
export async function postToXPlaywright(
	options: XPlaywrightOptions,
): Promise<void> {
	const {
		text,
		images = [],
	...
	console.log(
		`[x-playwright] Launching Chromium (${chromiumPath || "system"}, profile: ${profileDir})`,
	);
	...
	try {
		...
		if (!editorFound) {
			...
			await page
				.waitForSelector(EDITOR_SELECTORS[0], { timeout: 120_000 })
				.catch(() => {});
		}
		...
		if (submit) {
			...
			if (!buttonClicked) {
				...
				await page
					.click(BUTTON_SELECTORS[0], { timeout: 10000 })
					.catch(() => {});
			}
			await page.waitForTimeout(3000);
