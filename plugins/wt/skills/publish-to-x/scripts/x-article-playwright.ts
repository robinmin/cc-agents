...
const SELECTORS = {
    ...
    fileInput: ['[data-testid="fileInput"]', 'input[type="file"][accept*="image"]', 'input[type="file"]'],
    previewButton: [
                ...
                if (isVisible && isActuallyVisible) {
                    ...
                    foundModal = {
                        ...
                        hasPublish: buttonTexts.some(
                            (t) => t?.toLowerCase().includes('publish') || t?.toLowerCase().includes('发布'),
                        ),
...
export async function publishArticle(markdownPath: string, options: ArticleOptions = {}): Promise<void> {
    const {
        title: titleOverride,
        cover: coverPath,
        submit = false,
