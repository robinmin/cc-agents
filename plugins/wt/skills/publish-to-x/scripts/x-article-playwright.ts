...
import { getAutoSubmitPreference, getDefaultProfileDir } from "./x-utils.js";
...
async function handleLoginFlow(
	...
	// navSelector from config is reserved for future use
	// eslint-disable-next-line @typescript-eslint/no-unused-vars
	const _navSelector = config.navSelector ?? DEFAULT_NAV_SELECTOR;
...
async function getChromiumPath(): Promise<string | undefined> {
	try {
		...
		console.log(
			"[x-article-pw] Playwright Chromium not found, falling back to system Chrome",
		);
		return undefined;
	}
}
...
async function handleConfirmationDialog(page: Page): Promise<boolean> {
			...
			if (el) {
				...
				if (isVisible && isActuallyVisible) {
					...
					foundModal = {
						...
						hasPublish: buttonTexts.some(
							(t) =>
								t?.toLowerCase().includes("publish") ||
								t?.toLowerCase().includes("发布"),
						),
						hasConfirm: buttonTexts.some(
							(t) =>
								t?.toLowerCase().includes("confirm") ||
								t?.toLowerCase().includes("确认") ||
	...
	if (modalInfo) {
		console.log(
			`[x-article-pw] Found modal with selector: ${modalInfo.selector}`,
		);
		console.log(
			`[x-article-pw] Modal buttons: ${modalInfo.buttonTexts.join(", ")}`,
		);
...
export async function publishArticle(
	...
	console.log(
		`[x-article-pw] Launching Chromium (${chromiumPath || "system"}, profile: ${profileDir})`,
	);
	...
	try {
		...
		await page.goto(X_ARTICLES_URL, {
			waitUntil: "domcontentloaded",
			timeout: 60000,
		});
		...
		if (!loggedIn) {
			console.log(
				"[x-article-pw] Login required, waiting for user to login...",
			);
		...
		for (const selector of titleSelectors) {
			...
			if (el) {
				...
				if (value && value !== title) {
					console.log(
						`[x-article-pw] WARNING: Title changed to "${value}" after operations!`,
					);
				}
			}
		}
		...
		if (cover) {
			const coverPathAbs = path.resolve(
		...
		if (editorBody) {
			...
			try {
				...
				console.log(
					`[x-article-pw] DEBUG: Original content length: ${content.length} chars`,
				);
				console.log(
					`[x-article-pw] DEBUG: Original content preview: ${content.substring(0, 200)}...`,
				);

				// Dispatch paste event with HTML content
				await page.evaluate(
					(args: { selector: string; html: string }) => {
				...
				const insertedInfo = await page.evaluate((sel) => {
					...
					return {
						...
						endsWithExpected:
							fullText.trim().endsWith("参考来源") ||
							fullText.trim().endsWith("参考"),
						children: el.children?.length || 0,
					};
				}, editorBody);

				console.log(
					`[x-article-pw] DEBUG: Inserted HTML length: ${insertedInfo.innerHTML.length} chars`,
				);
				console.log(
					`[x-article-pw] DEBUG: Inserted text length: ${insertedInfo.innerText.length} chars`,
				);
				console.log(
					`[x-article-pw] DEBUG: Child elements: ${insertedInfo.children}`,
				);
				console.log(`[x-article-pw] DEBUG: Section checks:`);
				console.log(
					`[x-article-pw] DEBUG:   - Has 引言 (Intro): ${insertedInfo.hasIntro}`,
				);
				console.log(
					`[x-article-pw] DEBUG:   - Has 结论 (Conclusion): ${insertedInfo.hasConclusion}`,
				);
				console.log(
					`[x-article-pw] DEBUG:   - Has 参考来源 (References): ${insertedInfo.hasReferences}`,
				);
				console.log(
					`[x-article-pw] DEBUG:   - Has PM2: ${insertedInfo.hasPM2}`,
				);
				console.log(
					`[x-article-pw] DEBUG:   - Has launchd: ${insertedInfo.hasLaunchd}`,
				);
				console.log(
					`[x-article-pw] DEBUG: Text preview (first 200): ${insertedInfo.innerText.substring(0, 200)}...`,
				);
				console.log(
					`[x-article-pw] DEBUG: Text preview (last 100): ...${insertedInfo.innerText.slice(-100)}`,
				);

				const contentLength = insertedInfo.innerText.length;
				console.log(
					`[x-article-pw] Content after paste attempt: ${contentLength} chars`,
				);

				if (contentLength < 100) {
					console.log(
						"[x-article-pw] Paste didn't work, trying execCommand...",
					);
					...
					console.log(
						`[x-article-pw] Content after insertHTML: ${contentLength2} chars`,
					);
				}
			} catch (e) {
		...
		if (submit) {
			...
			for (const btn of allButtons) {
				try {
					...
					if (isVisible && text && text.trim().length > 0) {
						console.log(
							`[x-article-pw] Button: "${text.trim()}" - visible: ${isVisible}`,
						);
					}
				} catch {
					// Ignore
			...
			const publishBtnLocator = page.locator(
				'button:has-text("Publish"), [data-testid="publishButton"]',
			);
			...
			if (publishBtnCount > 0) {
				...
				console.log(
					`[x-article-pw] Publish button found: count=${publishBtnCount}, enabled=${isEnabled}, visible=${isVisible}`,
				);
				...
				for (const selector of errorSelectors) {
					...
					if (el && (await el.isVisible())) {
						...
						console.log(
							`[x-article-pw] Error/Warning element (${selector}): ${text?.substring(0, 100)}`,
						);
					}
				}
				...
				const titleInput = await page.$(
					'textarea[placeholder*="Add a title"], [data-testid*="title"]',
				);
				if (titleInput) {
					const titleValue = await titleInput.evaluate(
				...
				if (editor) {
					const editorText = await editor.evaluate(
						(el: HTMLElement) => el.innerText,
					);
					console.log(
						`[x-article-pw] Editor text length: ${editorText.length} chars`,
					);
					console.log(
						`[x-article-pw] Editor preview: ${editorText.substring(0, 100)}...`,
					);
				}
				...
				console.log(
					`[x-article-pw] Publish button disabled attr: ${disabledAttr}`,
				);
				...
				console.log(
					"[x-article-pw] Final attempt: filling title before publish...",
				);
				for (const selector of titleSelectors) {
					const el = await page.$(selector);
					if (el && (await el.isVisible())) {
						...
						await el.fill(title);
						await page.waitForTimeout(500);
						const newValue = await el.evaluate(
							(el: HTMLInputElement) => el.value,
						);
				...
				const publishBtnNow = await page.locator(
					'button:has-text("Publish"), [data-testid="publishButton"]',
				);
				const isEnabledNow = await publishBtnNow.isEnabled();
				console.log(
					`[x-article-pw] Publish button enabled after wait: ${isEnabledNow}`,
				);

				if (isEnabledNow) {
					console.log(
						"[x-article-pw] Publish button is ENABLED - clicking to publish!",
					);

					// Take screenshot before clicking for debugging
					await page.screenshot({ path: "/tmp/x-before-publish.png" });
					console.log(
						"[x-article-pw] Screenshot saved: /tmp/x-before-publish.png",
					);
					...
					try {
						...
						console.log(
							"[x-article-pw] Regular click failed, trying JavaScript...",
						);
					}
					...
					if (!url.includes("/edit/")) {
						...
						console.log(
							"[x-article-pw] URL still on edit page, trying JavaScript click...",
						);
						...
						if (!url.includes("/edit/")) {
							console.log(
								"[x-article-pw] Article published via JavaScript click!",
							);
						} else {
							...
							if (!url.includes("/edit/")) {
								console.log(
									"[x-article-pw] Article published via event dispatch!",
								);
							}
						}
					}
					...
					console.log(
						"[x-article-pw] Screenshot saved: /tmp/x-after-publish.png",
					);
					...
					if (!page.url().includes("/edit/")) {
						...
						console.log(
							"[x-article-pw] WARNING: URL unchanged after all click methods",
						);
						console.log(
							"[x-article-pw] Browser open for manual verification...",
						);
					}
				} else {
