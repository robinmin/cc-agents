...
export async function quotePost(options: QuoteOptions): Promise<void> {
	...
	try {
		...
		if (!retweetFound) {
			...
			if (!retweetButtonVisible)
				throw new Error(
					"Timed out waiting for tweet. Please log in first or check the tweet URL.",
				);
		}
		...
		if (submit) {
			...
			console.log(
				"[x-quote] Quote composed (preview mode). Add --submit to post.",
			);
			console.log(
				"[x-quote] Browser will stay open for 30 seconds for preview...",
			);
			await sleep(30_000);
		}
	} finally {
		if (cdp) {
			try {
				...
				console.debug(
					"[x-quote] Browser.close error (may already be closed):",
					error,
				);
			}
			cdp.close();
		...
		setTimeout(() => {
			if (!chrome.killed)
				try {
					...
					console.debug("[x-quote] SIGKILL error:", error);
				}
		}, 2_000).unref?.();
		try {
			chrome.kill("SIGTERM");
		} catch (error) {
			console.debug("[x-quote] SIGTERM error:", error);
		}
	}
}
