...
async function waitForPageReady(page: Page, timeoutMs = 20000): Promise<void> {
	...
	console.log(
		"[zenn-pw] Editor not detected via standard selectors, continuing anyway...",
	);
}
...
async function checkLoginStatus(page: Page): Promise<boolean> {
	...
	try {
		const isLoggedIn =
			(await page
				.locator('.user-avatar, .logout-button, [class*="user"]')
				.first()
				.count()) > 0;
		return isLoggedIn;
	} catch {
...
async function fillTitle(page: Page, title: string): Promise<void> {
	...
	const result = await trySelectors(page, ZENN_SELECTORS.titleInput, {
		timeout: 5000,
		visible: true,
	});
...
async function fillContent(page: Page, content: string): Promise<void> {
	...
	const result = await trySelectors(page, ZENN_SELECTORS.contentEditor, {
		timeout: 5000,
		visible: true,
	});
	if (!result.found || !result.locator) {
			...
			await page.evaluate((text) => {
				...
				if (win.editor?.setValue) {
					...
					const cm = document.querySelector(".CodeMirror") as unknown as {
						CodeMirror?: { setValue: (text: string) => void };
					};
					if (cm?.CodeMirror) cm.CodeMirror.setValue(text);
				}
			}, content);
	...
	try {
		...
		console.log(
			`[zenn-pw] contentEditable fill failed: ${e}. Trying Playwright fill...`,
		);
...
async function setType(page: Page, type: "tech" | "idea"): Promise<void> {
	...
	const result = await trySelectors(page, ZENN_SELECTORS.typeSelect, {
		timeout: 2000,
		visible: false,
	});
	if (!result.found || !result.locator) {
...
async function addTopics(page: Page, topics: string[]): Promise<void> {
	...
	const result = await trySelectors(page, ZENN_SELECTORS.topicsInput, {
		timeout: 2000,
		visible: false,
	});
	if (!result.found || !result.locator) {
 ...
 */
export async function publishToZennPlaywright(
	...
	try {
		...
		if (!isLoggedIn) {
			console.log(
				"[zenn-pw] Not logged in. Please log in to your Zenn account.",
			);
			console.log(
				"[zenn-pw] Waiting for login (manual intervention required)...",
			);
		...
		if (await postBtn.isVisible()) {
			...
			const newArticleLink = await page
				.locator(
					':text("文章を執筆する"), [href="/articles/new"], [href*="/articles/new"]',
				)
				.first();
