...
import { generateSlug, getWtProfileDir, type ParsedArticle, parseMarkdownFile, ZENN_URLS } from './zenn-utils.js';
...
async function waitForPageReady(page: Page, timeoutMs = 20000): Promise<void> {
    ...
    while (Date.now() - start < timeoutMs) {
        try {
            ...
            const editorResult = await trySelectors(page, ZENN_SELECTORS.contentEditor, {
                timeout: 2000,
                visible: true,
            });
    ...
    try {
        ...
        if (await postBtn.isVisible()) {
            ...
            const newArticleLink = await page
                .locator(':text("文章を執筆する"), [href="/articles/new"], [href*="/articles/new"]')
                .first();
