...
async function mediumApi<T = unknown>(
	endpoint: string,
	token: string,
	options?: RequestInit,
): Promise<T> {
...
export async function createPost(
	...
	return await mediumApi<MediumPostResponse>(`/users/${userId}/posts`, token, {
		method: "POST",
...
function parseHtmlFile(filePath: string): ParsedArticle {
	...
	if (!title) {
		throw new Error(
			"Title is required. Add a <title> tag to your HTML or use --title flag.",
		);
	}
...
async function main(): Promise<void> {
	...
	for (let i = 0; i < args.length; i++) {
		...
		if (arg === "--title" && args[i + 1]) title = args[++i];
			...
			if (status !== "public" && status !== "draft" && status !== "unlisted") {
				throw new Error(
					`Invalid status: ${status}. Must be: public, draft, or unlisted`,
				);
			}
			publishStatus = status;
		} else if (arg === "--canonical-url" && args[i + 1])
			canonicalUrl = args[++i];
	...
	const response = await createPost(user.id, {
		...
		contentFormat:
			markdownFile || (!htmlFile && content?.startsWith("#"))
				? "markdown"
				: "html",
		tags: allTags,
		publishStatus,
	...
	if (response.data.publishedAt) {
		...
		console.log(
			"[medium] Draft saved. Publish it at: " +
