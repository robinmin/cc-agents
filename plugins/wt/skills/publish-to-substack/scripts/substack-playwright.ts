...
async function isLoggedIn(page: Page): Promise<boolean> {
	...
	try {
		const signInBtn = await page
			.locator('a:text("Sign in"), button:text("Sign in")')
			.first();
	...
	if (
		url.includes("/publish/home") ||
		url.includes("/publish/post/") ||
		url.includes("/new")
	) {
		return true;
	}
	...
	try {
		const avatar = await page
			.locator(
				'[data-testid="user-menu-button"], .nav-user-menu, .profile-menu img',
			)
			.first();
	...
	try {
		const dashboardBtn = await page
			.locator('a:text("Dashboard"), a:text("Create Post")')
			.first()
			.isVisible({ timeout: 2000 });
...
async function fillTitle(page: Page, title: string): Promise<void> {
	...
	const result = await trySelectors(page, SUBSTACK_SELECTORS.titleInput, {
		timeout: 10000,
	});
...
async function fillSubtitle(page: Page, subtitle: string): Promise<void> {
	...
	const result = await trySelectors(page, SUBSTACK_SELECTORS.subtitleInput, {
		timeout: 5000,
	});

	if (!result.found || !result.locator) {
		console.log(
			"[substack-pw] Subtitle field not found (optional, skipping...)",
		);
		return;
	}
...
async function fillContent(page: Page, content: string): Promise<void> {
	...
	const result = await trySelectors(page, SUBSTACK_SELECTORS.contentEditor, {
		timeout: 10000,
	});
	...
	try {
		console.log(
			"[substack-pw] Attempting to fill content via HTML clipboard pasting...",
		);
		...
		// Verify paste result
		const textLength = await locator.evaluate(
		...
		console.log(
			"[substack-pw] Clipboard paste produced minimal content, trying fallbacks...",
		);
	} catch (e) {
	...
	try {
		const inserted = await page.evaluate(
			(sel, text) => {
				...
				const win = window as unknown as { view?: ProseMirrorView };
				const view =
					(el as unknown as { __view?: ProseMirrorView }).__view || win.view;
...
async function addTags(page: Page, tags: string[]): Promise<void> {
	...
	const result = await trySelectors(page, SUBSTACK_SELECTORS.tagsInput, {
		timeout: 5000,
	});
...
async function publishArticle(page: Page, asDraft: boolean): Promise<string> {
	...
	const result = await trySelectors(
		page,
		buttonSelectors as unknown as string[],
		{ timeout: 10000 },
	);
	...
	if (!result.found || !result.locator) {
		...
		console.log(
			"[substack-pw] Continue/Draft button not found, trying keyboard shortcut...",
		);
		...
		// Click the button
		console.log(
			`[substack-pw] Clicking ${asDraft ? "draft" : "continue"} button...`,
		);
	...
	if (!asDraft) {
		console.log(
			"[substack-pw] On settings page, clicking final publish button...",
		);

		// Check if on settings page via URL or presence of button
		const finalResult = await trySelectors(
			page,
			SUBSTACK_SELECTORS.finalPublishButton as unknown as string[],
			{ timeout: 10000 },
		);
		if (finalResult.found && finalResult.locator) {
			...
			console.log(
				"[substack-pw] Final publish button not found, checking if already published...",
			);
		}
	}
...
export async function publishToSubstack(
	...
	try {
		...
		const dashboardUrl =
			...
			"https://robinmin.substack.com/publish/home?utm_source=menu";
		...
		for (let attempt = 0; attempt < 3; attempt++) {
			...
			console.log(
				`[substack-pw] Current URL (attempt ${attempt + 1}): ${currentUrl}`,
			);
			...
			const finalUrl = page.url();
			if (
				finalUrl.includes("robinmin.substack.com") &&
				finalUrl.includes("/publish")
			) {
				console.log(
					"[substack-pw] Successfully reached publication dashboard.",
				);
				break;
			}

			if (
				finalUrl === "https://substack.com" ||
				finalUrl === "https://substack.com/" ||
				finalUrl.includes("substack.com/home")
			) {
				console.log(
					"[substack-pw] Landed on global home, clicking dashboard button...",
				);
				const dashboardBtn = await page
					.locator(
						'a:has-text("Dashboard"), button:has-text("Dashboard"), a[href*="/publish"]',
					)
					.first();
				if (await dashboardBtn.isVisible({ timeout: 5000 })) {
					...
					console.log(
						"[substack-pw] Dashboard button not found, force navigating...",
					);
					await page.goto("https://robinmin.substack.com/publish/home", {
				...
				console.log(
					"[substack-pw] Unexpected URL, force navigating to dashboard...",
				);
				await page.goto("https://robinmin.substack.com/publish/home", {
					waitUntil: "networkidle",
					timeout: 30000,
		...
		console.log(
			"[substack-pw] Attempting to create new article via dashboard...",
		);
		const createNewBtn = await trySelectors(
			page,
			SUBSTACK_SELECTORS.createNewButton as unknown as string[],
			{ timeout: 10000 },
		);
		if (createNewBtn.found && createNewBtn.locator) {
			...
			const articleItem = await trySelectors(
				page,
				[
					'a:has-text("Article")',
					'div[role="menuitem"]:has-text("Article")',
			...
			if (articleItem.found && articleItem.locator) {
				...
				try {
					...
					console.log(
						'[substack-pw] Navigation timeout after "Article" click, checking if on editor...',
					);
				}
				...
				if (page.url().includes("/publish/home")) {
					console.log(
						"[substack-pw] Still on dashboard after click, force navigating to /new...",
					);
				...
				console.log(
					'[substack-pw] "Article" menu item not found, trying fallback to /new...',
				);
				await page.goto(newPostUrl, {
					waitUntil: "domcontentloaded",
					timeout: 30000,
			...
			console.log(
				'[substack-pw] "Create new" button not found, trying fallback to /new...',
			);
			await page.goto(newPostUrl, {
				waitUntil: "domcontentloaded",
				timeout: 30000,
