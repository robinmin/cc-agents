...
import { buildEditorSelectors, trySelectors } from '@wt/web-automation/selectors';
...
async function isLoggedIn(page: Page): Promise<boolean> {
    ...
    try {
        const avatar = await page
            .locator('[data-testid="user-menu-button"], .nav-user-menu, .profile-menu img')
            .first();
    ...
    try {
        const inserted = await page.evaluate(
            (sel, text) => {
                ...
                const view = (el as unknown as { __view?: ProseMirrorView }).__view || win.view;
...
async function publishArticle(page: Page, asDraft: boolean): Promise<string> {
    ...
    if (!asDraft) {
        ...
        const finalResult = await trySelectors(page, SUBSTACK_SELECTORS.finalPublishButton as unknown as string[], {
            timeout: 10000,
        });
    ...
    try {
        ...
        for (let attempt = 0; attempt < 3; attempt++) {
            ...
            if (
                ...
                const dashboardBtn = await page
                    .locator('a:has-text("Dashboard"), button:has-text("Dashboard"), a[href*="/publish"]')
                    .first();
        ...
        const createNewBtn = await trySelectors(page, SUBSTACK_SELECTORS.createNewButton as unknown as string[], {
            timeout: 10000,
        });
