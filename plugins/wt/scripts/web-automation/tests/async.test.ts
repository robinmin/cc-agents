...
describe('async', () => {
    ...
    describe('withTimeout', () => {
        ...
        it('should throw on abort signal', async () => {
            ...
            await expect(withTimeout(Promise.resolve('never'), { signal: controller.signal })).rejects.toThrow(
                'aborted',
            );
        });
        ...
        it('should handle already aborted signal', async () => {
            ...
            await expect(withTimeout(Promise.resolve('test'), { signal: controller.signal })).rejects.toThrow(
                'aborted',
            );
        });
    });
    ...
    describe('retry', () => {
        ...
        it('should use shouldRetry predicate', async () => {
            ...
            await expect(retry(fn, { maxAttempts: 5, shouldRetry })).rejects.toThrow('Failed');
    ...
    describe('poll', () => {
        ...
        it('should timeout when condition is never met', async () => {
            ...
            await expect(poll(condition, { intervalMs: 10, timeoutMs: 50 })).rejects.toThrow(WtTimeoutError);
        });
