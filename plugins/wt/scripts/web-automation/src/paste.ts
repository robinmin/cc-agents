...
export async function activateApp(
    ...
    `;
        ...
        try {
            ...
            console.error(
                `[paste] activateApp error (attempt ${attempt}/${retries}): ${error instanceof Error ? error.message : error}`,
            );
            if (attempt < retries) {
                await sleepAsync(delayMs);
            }
    ...
    try {
        const fallbackResult = spawnSync('osascript', ['-e', `tell application "${appName}" to launch`], {
            stdio: 'pipe',
        });
