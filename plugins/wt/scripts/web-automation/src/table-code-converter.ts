...
function tableToHtml(table: TableBlock): string {
    ...
    let html =
        '<table style="border-collapse: collapse; width: 100%; font-family: -apple-system, BlinkMacSystemFont, \'Segoe UI\', sans-serif; font-size: 14px;">';
...
export function formatCodeBlockBordered(codeBlock: CodeBlock): string {
    ...
    const maxLen = Math.max(`[${language.toUpperCase()}]`.length, ...lines.map((l) => l.length));

    const border = '─'.repeat(maxLen + 2);

    const result = [`┌${border}┐`, `│ ${language.padEnd(maxLen)} │`, `├${border}┤`];
...
export async function renderTableAsImage(
    ...
    try {
        ...
        const {
            result: { value: dims },
        } = await cdp.send<{ result: { value: { width: number; height: number } } }>(
            'Runtime.evaluate',
            {
                expression: `(() => {
        ...
        // Capture screenshot
        const { data: screenshotData } = await cdp.send<{ data: string }>(
...
export async function convertTablesAndCodeBlocks(
    ...
    while (i < lines.length) {
        ...
        if (line.startsWith('```')) {
            ...
            if (codeBlock) {
                const converted = formatCodeBlocks ? formatCodeBlockBordered(codeBlock) : codeBlock.markdown;
