...
> **Note:** This is the ast-grep (`sg`) rule format. For ripgrep (`rg`) patterns, use standard regex — see `decision-tree.md`.
...
- [Metavariables](#metavariables)
  ...
  - [$$$MULTI_META_VARIABLE: Multi-Node Capture](#multimeta_variable-multi-node-capture)
...
Metavariables starting with an underscore (`_`) are non-capturing/anonymous metavariables. They do not capture matched content and can match different content even if named identically. This is useful when you need the pattern structure but don't need to capture the matched values.

* **Example**: `$_FUNC($_FUNC)` matches `test(a)`, `testFunc(1 + 1)`, and `func(...args)`.
* **Reason**: `$_FUNC` can be "test" in the first position and "a" in the second - they don't need to match.
* **Use when**: You want to match pattern structure without capturing values, or allow same variable name to match different content.
* **Performance**: Can optimize matching by avoiding unnecessary captures.
