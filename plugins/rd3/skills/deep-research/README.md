...
Reports saved to `~/tcc/[Topic]_Research_[Date]/`:
...
├── scripts/
│   ├── validate_report.ts            # 9-check structure validator
│   ├── verify_citations.ts           # DOI/URL/hallucination checker
│   ├── citation_manager.ts           # Citation tracking & source credibility scoring
│   ├── md_to_html.ts                 # Markdown to HTML converter
│   ├── verify_html.ts                # HTML verification
│   └── research_engine.ts            # Core orchestration engine
└── tests/
