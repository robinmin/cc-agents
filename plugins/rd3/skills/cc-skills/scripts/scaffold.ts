...
import type { InteractionPattern, Platform, ResourceType, ScaffoldOptions } from './types';
...
const ALLOWED_PLATFORMS: readonly Platform[] = ['claude', 'codex', 'openclaw', 'opencode', 'antigravity'];
...
export function example(): string {
  return 'Example script for ${skillName}';
}
`;
 ...
 */
function generateOpenaiYaml(skillName: string, _skillTitle: string, description: string): string {
 ...
 */
function printNextSteps(skillDir: string, resources: string[], examples: boolean, platforms: Platform[]): void {
    ...
    const platformFollowUps: string[] = [];
    if (platforms.includes('codex')) {
        platformFollowUps.push('Update agents/openai.yaml if the Codex UI metadata should differ');
    }
    if (platforms.includes('openclaw')) {
        platformFollowUps.push('Review metadata.openclaw in SKILL.md frontmatter for OpenClaw-specific details');
    }
    if (platforms.some((platform) => ['claude', 'opencode', 'antigravity'].includes(platform))) {
        platformFollowUps.push('Adjust Platform Notes in SKILL.md if behavior differs by target platform');
    }

    if (platformFollowUps.length > 0) {
        logger.log(`2. ${platformFollowUps.join(' ')}`);
    } else {
        logger.log('2. Review platform-specific metadata in SKILL.md if needed');
    }
...
function printUsage(): void {
    ...
    logger.log(
        '  --platform <name[,name...]> Target platform(s): all, claude, codex, openclaw, opencode, antigravity (default: all)',
    );
...
function parseCliArgs(): ScaffoldCliOptions {
    ...
    if (values.platform && typeof values.platform === 'string') {
        ...
        const invalidPlatforms = requestedPlatforms.filter(
            (platform) => platform !== 'all' && !ALLOWED_PLATFORMS.includes(platform as Platform),
        );

        if (invalidPlatforms.length > 0) {
            logger.error(`Unknown platform(s): ${invalidPlatforms.join(', ')}`);
            logger.error(`Allowed: all, ${ALLOWED_PLATFORMS.join(', ')}`);
            process.exit(1);
        }

        options.platforms =
            requestedPlatforms.length === 0 || requestedPlatforms.includes('all')
                ? [...ALLOWED_PLATFORMS]
                : (Array.from(new Set(requestedPlatforms)) as Platform[]);
    }
...
async function main() {
    ...
    if (!options.name) {
        ...
        logger.error('  --platform <name[,name...]>   Target platform(s): all, claude, codex, openclaw (default: all)');
    ...
    logger.log(`   Platforms: ${options.platforms.join(', ')}`);
    logger.log('');
    ...
    if (skillDir) {
        printNextSteps(skillDir, options.resources || [], options.examples || false, options.platforms);
        process.exit(0);
    } else {
        process.exit(1);
    }
