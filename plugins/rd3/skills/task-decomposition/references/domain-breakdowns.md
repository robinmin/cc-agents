---
name: domain-breakdowns
description: "Domain-specific task breakdown patterns for: authentication, API development, database migrations, frontend features, and CI/CD pipelines."
see_also:
...
These breakdowns assume requirements, architecture decisions, and major system constraints are
already understood. If those inputs are missing, treat them as prerequisites or open questions
rather than solving them inside task decomposition.
...
```
1. Confirm authentication architecture inputs and constraints
...
```
1. Confirm OAuth2 integration constraints and chosen provider strategy
...
```
1. Confirm MFA constraints and selected factors (TOTP, SMS, backup codes)
...
```
1. Confirm API contract inputs (OpenAPI/Swagger or equivalent)
...
```
1. Confirm schema migration inputs (forward + backward compatibility, rollback needs)
...
X No Testing in Staging:
   Deploy directly to production
   (Dangerous for schema changes)

OK Incremental Migration:
...
```
1. Confirm component boundaries and UI constraints
...
```
1. Confirm state management constraints (local vs global)
...
```
1. Confirm pipeline stages, environments, and deployment constraints
...
10. Document container deployment constraints and conventions
```
