...
function makeFakeQueries(
  history?: Array<{ taskRef: string; runId?: string }>,
) {
  return {
...
describe("handleEvents — invalid event type exits with code 10", () => {
  ...
  test("single invalid type", async () => {
    ...
    const queries = makeFakeQueries([
      { taskRef: "0300", runId: "run-001" },
    ]);

    try {
      await handleEvents(
        { run: "run-001", types: ["bogus.type"] },
        sm,
        queries as never,
      );
    } catch {
  ...
  test("partial invalid type in comma-separated list", async () => {
    ...
    const queries = makeFakeQueries([
      { taskRef: "0300", runId: "run-002" },
    ]);

    try {
      await handleEvents(
        { run: "run-002", types: ["run.created", "invalid.phase"] },
        sm,
        queries as never,
      );
    } catch {
...
describe("handleEvents — text table format", () => {
  ...
  test("--phase filter outputs only matching events", async () => {
    ...
    const queries = makeFakeQueries([
      { taskRef: "0300", runId: "run-phase" },
    ]);

    try {
      await handleEvents(
        { run: "run-phase", phase: "develop" },
        sm,
        queries as never,
      );
    } catch {
  ...
  test("--type filter outputs only matching events", async () => {
    ...
    const queries = makeFakeQueries([
      { taskRef: "0300", runId: "run-type" },
    ]);

    try {
      await handleEvents(
        { run: "run-type", types: ["run.created", "run.started"] },
        sm,
        queries as never,
      );
    } catch {
  ...
  test("resolves taskRef from history when --run not provided", async () => {
    ...
    const queries = makeFakeQueries([
      { taskRef: "docs/tasks2/0300_test.md", runId: "run-resolved" },
    ]);
  ...
  async function setupJsonTest(runId: string, taskRef: string, events?: Parameters<typeof EventStore.prototype.append>[0][]) {
  ...
  test("handleEvents > json output with events", async () => {
    const testSm = await setupJsonTest("run-json", "0300", [
      { run_id: "run-json", event_type: "phase.started", payload: { phase_name: "develop", fromState: "pending", toState: "running" } },
      { run_id: "run-json", event_type: "phase.completed", payload: { phase_name: "develop", fromState: "running", toState: "completed" } },
    ]);
    ...
    try {
      await handleEvents(
        { taskRef: "0300", run: "run-json", json: true },
        testSm,
        queries as never,
      );
    } catch {
  ...
  test("handleEvents > json output with empty events", async () => {
    ...
    try {
      await handleEvents(
        { taskRef: "0301", run: "run-empty-json", json: true },
        testSm,
        queries as never,
      );
    } catch {
  ...
  test("handleEvents > json output with type filter", async () => {
    const testSm = await setupJsonTest("run-type-json", "0302", [
      { run_id: "run-type-json", event_type: "phase.started", payload: { phase_name: "develop", fromState: "pending", toState: "running" } },
      { run_id: "run-type-json", event_type: "phase.completed", payload: { phase_name: "develop", fromState: "running", toState: "completed" } },
    ]);
  ...
  test("handleEvents > json output with phase filter", async () => {
    const testSm = await setupJsonTest("run-phase-json", "0303", [
      { run_id: "run-phase-json", event_type: "phase.started", payload: { phase_name: "develop", fromState: "pending", toState: "running" } },
      { run_id: "run-phase-json", event_type: "phase.started", payload: { phase_name: "plan", fromState: "pending", toState: "running" } },
    ]);
  ...
  test("handleEvents > json output with no matching type filter", async () => {
    const testSm = await setupJsonTest("run-no-match-json", "0304", [
      { run_id: "run-no-match-json", event_type: "phase.started", payload: { phase_name: "develop", fromState: "pending", toState: "running" } },
    ]);
