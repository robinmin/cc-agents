...
async function formatEventsOutput(
  ...
  if (typeFilters.length > 0)
    process.stdout.write(`Filter: --type ${typeFilters.join(",")}\n`);
  if (phaseFilter)
    process.stdout.write(`Filter: --phase ${phaseFilter}\n`);
  process.stdout.write(`Total events: ${events.length}\n\n`);

  for (const e of events) {
    const seq = String(e.sequence ?? "?").padStart(4);
    const ts = e.timestamp
      ? e.timestamp.toISOString().slice(0, 19).replace("T", " ")
      : "             ";
    const type = e.event_type.padEnd(24);
    const phase = ((e.payload.phase_name as string | undefined) ?? "").padEnd(
      16,
    );
    const transition =
...
export async function handleEvents(
  ...
  if (!resolvedRunId && taskRef) {
    ...
    const match = history.find(
      (h) => normalizeTaskRef(h.taskRef) === taskRef,
    );
  ...
  if (typeFilters.length > 0) {
    for (const t of typeFilters) {
      if (!VALID_EVENT_TYPES.includes(t as EventType)) {
        logger.error(
          `Unknown event type: ${t}. Valid types: ${VALID_EVENT_TYPES.join(", ")}`,
        );
        process.exit(EXIT_INVALID_ARGS);
      }
    }
  ...
  const filtered = phaseFilter
    ? events.filter(
        (e) => (e.payload.phase_name as string) === phaseFilter,
      )
    : events;
