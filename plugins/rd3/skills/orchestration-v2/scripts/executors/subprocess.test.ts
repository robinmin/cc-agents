	...
	describe("buildArgs", () => {
		...
		it("serializes payload values as JSON for non-string types", () => {
			...
			const args = exec.buildArgs(
				makeRequest({ payload: { count: 42, flag: true, name: "test" } }),
			);
	...
	describe("execute", () => {
		...
		it("captures subprocess failure exit code", async () => {
			...
			createRunScript(
				...
				"rd3",
				"fail-skill",
				`process.stderr.write("script error"); process.exit(1);`,
			);
		...
		it("executes index.ts when no scripts/run.ts exists", async () => {
			...
			writeFileSync(
				join(dir, "index.ts"),
				`process.stdout.write("from index"); process.exit(0);`,
			);
