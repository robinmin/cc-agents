/**
 * ftree — State Machine Tests
 *
 * R8.2: Every valid transition accepted, every invalid transition rejected.
 */
...
import {
    ...
    describeTransition,
    TRANSITION_MAP,
} from '../scripts/lib/state-machine';
import type { FeatureStatus } from '../scripts/types';
...
describe('State Machine', () => {
    describe('validateTransition', () => {
        ...
        test('executing → validated is invalid', () => {
            expect(validateTransition('executing', 'validated')).toBe(false);
        });
        ...
        test('done → executing is invalid', () => {
            expect(validateTransition('done', 'executing')).toBe(false);
        });

        test('done → validated is invalid', () => {
            expect(validateTransition('done', 'validated')).toBe(false);
        });
