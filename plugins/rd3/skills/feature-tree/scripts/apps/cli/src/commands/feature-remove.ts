...
export class FeatureRemoveCommand extends Command {
    constructor() {
        super();
    }
    ...
    static usage = Command.Usage({
        ...
        description: 'Remove a feature (alias for delete)',
        examples: [
            ['Remove a leaf feature', 'ftree remove abc123'],
            ['Force remove with children', 'ftree remove abc123 --force'],
        ],
    });
    ...
    force = Option.Boolean('--force,-f', false, {
        description: 'Force removal even if the feature has children',
    });
    ...
    async execute() {
        ...
        try {
            ...
            this.context.stdout.write(`Removed ${result.data.count} feature(s)\n`);
            return 0;
        } catch (e) {
