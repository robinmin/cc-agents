...
import type { HandoverOptions, HandoverDocument, GitDiff, TaskFileContext } from "./types";
import { generateHandoverMarkdown, slugify } from "./template";

const HANDOVER_DIR = "docs/handovers";
...
function parseArgs(): HandoverOptions {
  ...
  for (let i = 0; i < args.length; i++) {
    ...
    } else if (arg === "--blocker" && i + 1 < args.length) {
      options.blocker = args[++i];
    } else if (arg === "--next-steps" && i + 1 < args.length) {
...
export async function buildHandoverDocument(options: HandoverOptions): Promise<HandoverDocument> {
  ...
  const doc: HandoverDocument = {
    ...
    rejectedApproaches: options.rejectedApproaches || [],
...
export async function main(): Promise<void> {
  ...
  if (!options.description) {
    logger.error("Description is required. Usage: handover.ts <description> [--task <file>]");
    process.exit(1);
  }

  ensureDir(HANDOVER_DIR);

  const doc = await buildHandoverDocument(options);
  const markdown = generateHandoverMarkdown(doc);

  const slug = slugify(options.description);
  const filename = `${new Date().toISOString().slice(0, 10)}-handover-${slug}.md`;
  const filepath = `${HANDOVER_DIR}/${filename}`;

  writeFileSync(filepath, markdown, "utf-8");

  logger.info(`Handover document created: ${filepath}`);
  console.log(`\n${markdown}\n`);
}
