...
export function App() {
  ...
  return (
    <div className="min-h-screen" style={{ background: "var(--kanban-bg)" }}>
      {connected === false && (
        <div className="px-4 py-1.5 text-xs text-white text-center font-medium" style={{ background: "var(--kanban-danger)" }}>
          ...
          <div className="flex items-center gap-2 px-2 py-1 bg-gray-50 dark:bg-gray-800/50 rounded-lg border border-gray-200 dark:border-gray-700">
            {STATUS_ORDER.map((status) => (
              ...
              >
                ...
                <span className="text-[10px] font-bold uppercase tracking-wider hidden xl:inline" style={{ color: "var(--kanban-text-secondary)" }}>
                  {status}
                </span>
              </label>
            ))}
      ...
      {loading ? (
        <div className="flex items-center justify-center h-64" style={{ color: "var(--kanban-text-secondary)" }}>
          Loading tasks...
        </div>
      ) : (
        <KanbanBoard
      ...
      {selectedWbs && (
        <TaskDetail wbs={selectedWbs} onClose={() => setSelectedWbs(null)} />
      )}

      {showCreate && (
        <TaskCreate onClose={() => setShowCreate(false)} />
      )}
    </div>
  );
}
