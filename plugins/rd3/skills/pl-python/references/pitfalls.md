---
name: pitfalls
description: "Common Python mistakes and their solutions"
see_also:
  - rd3:pl-python
  - rd3:sys-debugging
---
...
def process_data(data):
    ...
    print("First pass:", list(filtered_items()))  # [2]
    print("Second pass:", list(filtered_items()))  # [2]
```
...
- Use linters (ruff) to catch shadowing
...
async def parent():
    await asyncio.run(child())  # Error: asyncio.run() cannot be called from a running event loop
