...
export function detectConflicts(sources: SourceContent[]): ConflictManifest {
  ...
  for (const [filePath, pathSources] of Object.entries(byPath)) {
    ...
    const allSame = pathSources.every((s) =>
      isEffectivelySame(s.content, pathSources[0].content),
    );
  ...
  for (const [, sectionEntries] of Object.entries(sectionIndex)) {
    ...
    const allSectionsSame = contents.every((c) =>
      isEffectivelySame(c, contents[0]),
    );
    ...
    if (!allSectionsSame) {
      const conflict: Conflict = {
        ...
        snippets: Object.fromEntries(
          sectionEntries.map((e) => [e.source.sourceName, e.section.content]),
        ),
        resolution: "",
        attribution: {},
      };
  ...
  for (const [, entries] of Object.entries(paragraphIndex)) {
    ...
    const allParasSame = contents.every((c) =>
      isEffectivelySame(c, contents[0]),
    );
    ...
    if (!allParasSame) {
      const conflict: Conflict = {
        ...
        snippets: Object.fromEntries(
          entries.map((e) => [e.source.sourceName, e.paragraph.content]),
        ),
        resolution: "",
        attribution: {},
      };
  ...
  for (const [, entries] of Object.entries(paragraphIndex)) {
    ...
    for (let lineIdx = 0; lineIdx < firstLines.length; lineIdx++) {
      ...
      const allLinesSame = lineContents.every(
        (l) => l === lineContents[0] || l.trim() === "",
      );
