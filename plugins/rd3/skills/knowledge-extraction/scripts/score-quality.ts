...
function scoreCompleteness(
  ...
  const mentionedSources = sources.filter((s) => {
    ...
    return (
      ...
      Object.values(conflictAttributions).some((v) =>
...
function scoreTraceability(
  ...
  const hasAttribution =
    /source:/i.test(merged) ||
    /according to/i.test(merged) ||
    /from.*\(/i.test(merged);
  ...
  const hasConflictNote =
    /conflict/i.test(merged) ||
    /differ/i.test(merged) ||
    /resolution/i.test(merged);
...
export function scoreMergeQuality(
  ...
  if (dimensions.coherence < 10) {
    warnings.push(
      "Coherence is very low: merged text may be fragmented or contradictory.",
    );
  }
  if (dimensions.completeness < 10) {
    warnings.push(
      "Completeness is low: some source content may have been lost.",
    );
  }
  if (dimensions.nonRedundancy < 10) {
    warnings.push(
      "Non-redundancy is low: significant duplicate content detected.",
    );
  }
