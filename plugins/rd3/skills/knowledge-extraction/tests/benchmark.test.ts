 ...
 * Tests conflict detection and quality scoring across all conflict types.
 */
...
const BENCHMARK_CASES: BenchmarkCase[] = [
...

  {
    ...
    description: "Same code block, one line differs - detected as file-level conflict since content differs significantly",
    conflictTypes: ["file"],
    sources: [
      {
        name: "version-1",
...
function runConflictDetectionBenchmark(
  ...
  try {
    ...
    // Score quality of the merged content simulation
    const mergedContent = simulateMergedContent(testCase.sources, manifest);
    const qualityScore = scoreMergeQuality(
      mergedContent,
      testCase.sources,
    );
    ...
    return {
      ...
      qualityJustification: qualityScore.justification,
      conflictsDetected,
      conflictsResolved: conflictsDetected, // All detected are considered "resolved" in benchmark
      deterministic: testCase.deterministic ?? true,
      error: passed ? undefined : `Expected ${[...expectedTypes]}, got ${[...detectedTypes]}`,
    };
  } catch (error) {
    return {
      caseId: testCase.id,
...
/**
 * Simulate merged content for quality scoring purposes.
 * This is a simple deterministic merge for benchmark testing.
 */
function simulateMergedContent(
  sources: SourceContent[],
  _manifest: ConflictManifest,
): string {
  // Sort sources alphabetically for deterministic merge
  const sortedSources = [...sources].sort((a, b) =>
    a.name.localeCompare(b.name),
  );

  const lines: string[] = [];
  lines.push("# Merged Document\n");

  // Group by file path
  const byFile: Record<string, SourceContent[]> = {};
  for (const source of sortedSources) {
    if (!byFile[source.path]) byFile[source.path] = [];
    byFile[source.path].push(source);
  }

  for (const [filePath, fileSources] of Object.entries(byFile)) {
    lines.push(`\n## File: ${filePath}\n`);

    if (fileSources.length === 1) {
      // No conflict, use as-is
      lines.push(fileSources[0].content);
    } else {
      // Has conflicts, combine all content
...
describe("knowledge-extraction benchmark", () => {
  ...
  for (const testCase of BENCHMARK_CASES) {
    test(testCase.id, () => {
      const result = runConflictDetectionBenchmark(testCase);
      results.push(result);
      ...
      if (!result.passed) {
        ...
        console.log(`PASS: ${testCase.id} (quality: ${result.qualityScore}/100)`);
      }

      // Conflict detection must pass
  ...
  test("benchmark summary", () => {
    ...
    console.log("Note: Quality scores are based on simulated merge (concatenation).");
    console.log("      Real quality scores require reconcile.ts implementation.");
    console.log("========================================\n");

    // Conflict detection requires ≥80% pass rate
...
`;
    ...
    const result = scoreMergeQuality(content, sources);
    expect(result.dimensions.nonRedundancy).toBeLessThan(15);
  });
...
`;
    ...
    const sources: SourceContent[] = [
      { name: "src-a", path: "doc.md", content: "# Complete\n\n## Overview\n\nFull content.\n\n## Details\n\nMore details here." },
      { name: "src-b", path: "doc.md", content: "# Complete\n\n## Overview\n\nFull content.\n\n## Details\n\nMore details here." },
    ];
