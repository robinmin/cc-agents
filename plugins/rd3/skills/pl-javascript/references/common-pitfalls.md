...
description: "Guide to common JavaScript mistakes: scope issues, type coercion, async pitfalls, memory leaks, and how to avoid them."
see_also:
  - rd3:pl-javascript
---
...
// With AbortController for multiple timers
const controller = new AbortController();
setTimeout(handler, 1000, { signal: controller.signal });
controller.abort();  // Clears all timers with this signal
```
