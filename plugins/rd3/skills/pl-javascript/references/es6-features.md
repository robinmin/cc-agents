---
name: es6-features
description: "Complete guide to modern JavaScript (ES6+) features: variables, arrow functions, destructuring, spread, classes, modules, iterators, and ES2022-2026 additions."
see_also:
  - rd3:pl-javascript
  - rd3:pl-typescript
---
...
- [SOTA Features](#sota-features-es2022-2026)
...
// Find
const found = generateNumbers().find(x => x > 2);
...
// Map and group
const groupedNames = Object.groupBy(
  inventory,
  item => item.type
).map((type, items) => ({ type, names: items.map(i => i.name) }));
```
...
// Unlike 0.1 + 0.2 + 0.3 which gives 0.6000000000000001
```
...
```javascript
// In ESM modules - no async wrapper needed
const data = await fetch('/api/data').then(r => r.json());
const config = await import('./config.js');

// Useful for module-level initialization
const cache = new Map();
export const init = await fetch('/api/config').then(r => r.json());
```
