...
export function detectDataSourceAvailability(targetPath: string): Record<EvolutionDataSource, boolean> {
  const resolvedTarget = resolve(targetPath);
  const startDir = existsSync(resolvedTarget) && !resolvedTarget.endsWith(".md")
    ? resolvedTarget
  ...
  return {
    ...
    "user-feedback": existsSync(join(workspaceRoot, "FEEDBACK.md")) || existsSync(join(workspaceRoot, "feedback.md")),
...
export function generateRefineBackedProposals(
  ...
  if (analysis.patterns.length === 0) {
    proposals.push({
      ...
      rationale: "No immediate weaknesses were found, but the evolve surface should still record a periodic review proposal.",
      source: pickSource(report.targetPath),
      confidence: 0.5,
      affectsCritical: false,
      action: {
...
export function getNextVersionId(
  versions: Array<EvolutionVersionSnapshot<string>>,
): string {
...
export function restoreTargetedSnapshot(
...
export function saveSnapshotBackup(
  ...
  const backupPath = join(
    storage.backupsDir,
    `${getTargetKey(targetPath)}-${label}-${Date.now()}.snapshot.json`,
  );
...
export function createBackup(namespace: string, targetPath: string, content: string): string {
  ...
  const backupPath = join(
    storage.backupsDir,
    `${getTargetKey(targetPath)}-${Date.now()}.bak`,
  );
