...
# Normalize stage name: case-insensitive + aliases → canonical stage
# Returns canonical stage name or empty string if invalid
function normalize_stage() {
    local input="$1"
    local lower=$(echo "$input" | tr '[:upper:]' '[:lower:]')

    # Map aliases and case variations to canonical stages
    case "$lower" in
        backlog)
            echo "Backlog" ;;
        todo|to-do|"to do")
            echo "Todo" ;;
        wip|"in progress"|inprogress|"in-progress"|working)
            echo "WIP" ;;
        testing|test|review|"in review"|"in-review")
            echo "Testing" ;;
...
function validate_stage() {
    local stage="$1"
    local normalized=$(normalize_stage "$stage")

    if [ -z "$normalized" ]; then
        log_error "Invalid stage: '$stage'."
        log_error "Valid stages: ${VALID_STAGES[*]}"
        log_error "Aliases: todo/to-do, wip/in-progress/working, testing/test/review, done/completed/finished/closed"
        exit 1
    fi
...
function cmd_list() {
    ...
    if [ -z "$stage" ]; then
        ...
        # Normalize stage (validate_stage exits on error, returns canonical name)
        local normalized_stage=$(validate_stage "$stage")
        ...
        if ! grep -q "## $normalized_stage" "$KANBAN_FILE"; then
            log_error "Stage '$normalized_stage' not found in Kanban board."
