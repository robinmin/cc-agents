...
When building plugin ecosystems, follow the **Fat Skills, Thin Wrappers** pattern:

- **Skills** contain all core logic, workflows, and domain knowledge
- **Commands** (slash commands) are minimal wrappers that invoke skills for human users
- **Agents** (subagents) are minimal wrappers that invoke skills for AI-driven workflows
...
│  │ (workflow)│ (tools)   │ (docs)    │(templates)│  │
│  └───────────┴───────────┴───────────┴───────────┘  │
└─────────────────────────────────────────────────────┘
        ▲                           ▲
        │                           │
┌───────┴───────┐           ┌───────┴───────┐
│ THIN COMMAND  │           │  THIN AGENT   │
│ (human invoke)│           │ (AI invoke)   │
│  ~50 lines    │           │  ~100 lines   │
└───────────────┘           └───────────────┘
```
...
- Commands and agents stay maintainable
- Updates to skill automatically propagate
...
3. Initialize the skill (run `scripts/skills.py init`)
4. Edit the skill (implement resources and write SKILL.md)
5. Package the skill (run `scripts/skills.py package`)
...
When creating a new skill from scratch, always run the `scripts/skills.py init` command. This conveniently generates a new template skill directory that automatically includes everything a skill requires, making the skill creation process much more efficient and reliable.
...
- **Comprehensive best practices**: See [references/best_practices.md](references/best_practices.md) for the complete guide
...
- **Security guidelines**: See [references/security.md](references/security.md) for security checklist
- **Evaluation-first development**: See [references/evaluation.md](references/evaluation.md) for test-driven skill creation
...
```bash
scripts/skills.py package <path/to/skill-folder>
```

Optional output directory specification:

```bash
scripts/skills.py package <path/to/skill-folder> ./dist
```
