#!/usr/bin/env python3
"""
OpenCode Code Review Utility - CLI tool for OpenCode-powered code review and planning.

Usage:
    python3 code-review-opencode.py <command> [options]
...
Examples:
    python3 code-review-opencode.py check
    python3 code-review-opencode.py run "Explain this function"
    python3 code-review-opencode.py run-file prompt.txt
    python3 code-review-opencode.py review src/auth/ --output plan.md
"""
...
def generate_temp_path(prefix: str = "opencode") -> Path:
...
OpenCode CLI is required for code review operations.
Please install OpenCode from: https://opencode.ai/docs/cli/

After installation, ensure 'opencode' is available in your PATH.""",
        )
    ...
    try:
        ...
        if result.returncode == 0:
            ...
            # Even if --version fails, if opencode exists, consider it available
            return CheckResult(
                available=True,
                message="opencode ready (version unknown)",
                version=None,
            )
        ...
        # Even if there's an exception, if opencode exists, consider it available
        return CheckResult(
            available=True,
            message="opencode ready",
            version=None,
...
def run_opencode_prompt(
    ...
    """
    Run a short prompt via OpenCode CLI.
    ...
    """
    try:
        # Use OpenCode's run command in non-interactive mode
        result = subprocess.run(
        ...
        return RunResult(
            success=False,
            output="",
            error=f"OpenCode CLI timeout after {timeout} seconds",
        )
...
def cmd_run(args: argparse.Namespace) -> int:
    ...
    check_result = check_opencode_availability()
...
def run_opencode_from_file(
    prompt_file: Path,
    timeout: int = TIMEOUT_MODERATE,
    output_file: Path | None = None,
) -> RunResult:
    """
    Run a long prompt from a file via OpenCode CLI.
    ...
    try:
        result = subprocess.run(
            [OPENCODE_CLI, "run", prompt_content],
            capture_output=True,
            text=True,
            timeout=timeout,
        )
...
def extract_review_sections(content: str) -> dict[str, str]:
    """
    Extract structured sections from OpenCode's review output.

    Args:
        content: Raw review output from OpenCode.
    ...
    for line in lines:
        ...
        if is_header and ("executive summary" in line_lower or ("summary" in line_lower and current_section is None)):
            ...
            current_content = []
        elif is_header and "critical" in line_lower and "must fix" in line_lower:
            if current_section and current_content:
                sections[current_section] = "\n".join(current_content).strip()
            current_section = "critical_issues"
            current_content = []
        elif is_header and "high priority" in line_lower and "should fix" in line_lower:
            if current_section and current_content:
                sections[current_section] = "\n".join(current_content).strip()
            current_section = "high_priority_issues"
            current_content = []
        elif is_header and "medium priority" in line_lower and "consider" in line_lower:
            if current_section and current_content:
                sections[current_section] = "\n".join(current_content).strip()
            current_section = "medium_priority_issues"
            current_content = []
        elif is_header and "low priority" in line_lower and "nice to have" in line_lower:
            if current_section and current_content:
                sections[current_section] = "\n".join(current_content).strip()
            current_section = "low_priority_issues"
            current_content = []
        elif is_header and "security" in line_lower and "analysis" in line_lower:
            if current_section and current_content:
                sections[current_section] = "\n".join(current_content).strip()
            current_section = "security_analysis"
            current_content = []
        elif is_header and "performance" in line_lower and "review" in line_lower:
            if current_section and current_content:
                sections[current_section] = "\n".join(current_content).strip()
            current_section = "performance_review"
            current_content = []
        elif is_header and "code quality" in line_lower:
            if current_section and current_content:
                sections[current_section] = "\n".join(current_content).strip()
            current_section = "code_quality"
            current_content = []
        elif is_header and "testing" in line_lower and "gap" in line_lower:
            if current_section and current_content:
                sections[current_section] = "\n".join(current_content).strip()
            current_section = "testing_gaps"
            current_content = []
        elif is_header and "strength" in line_lower:
            if current_section and current_content:
                sections[current_section] = "\n".join(current_content).strip()
            current_section = "strengths"
            current_content = []
        elif is_header and ("improvement" in line_lower or "area" in line_lower):
            if current_section and current_content:
                sections[current_section] = "\n".join(current_content).strip()
            current_section = "improvements"
            current_content = []
        elif is_header and ("follow-up" in line_lower or "followup" in line_lower):
            if current_section and current_content:
                sections[current_section] = "\n".join(current_content).strip()
            current_section = "followup_actions"
            current_content = []
        elif is_header and "next step" in line_lower:
            if current_section and current_content:
                sections[current_section] = "\n".join(current_content).strip()
...
def gather_code_content(target: str) -> tuple[str, list[Path]]:
    """
    Gather code content from a target path.

    Args:
        target: File or directory path.

    Returns:
        Tuple of (combined content, list of files).
    """
...
def cmd_review(args: argparse.Namespace) -> int:
    ...
    # Gather code content
    print(f"Gathering code from: {target}", file=sys.stderr)
    code_content, files = gather_code_content(target)

    if not files:
        print(f"Error: No files found for target: {target}", file=sys.stderr)
        return 1

    print(f"Found {len(files)} file(s) to review", file=sys.stderr)
    ...
    prompt_file = generate_temp_path("opencode-prompt")
    ...
    print(f"Running OpenCode ({mode} mode)...", file=sys.stderr)
...
def create_task_from_issue(issue: ReviewIssue, review_metadata: dict[str, str]) -> tuple[bool, str]:
    ...
    try:
        ...
        return False, f"Task creation timeout for: {task_name}"
    except Exception as e:
...
def main() -> int:
    ...
    parser = argparse.ArgumentParser(
        description="OpenCode Code Review Utility - CLI tool for OpenCode-powered code review and planning.",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Commands:
  check       Validate OpenCode CLI availability
  run         Run a short prompt via OpenCode CLI
...
Examples:
  python3 code-review-opencode.py check
  python3 code-review-opencode.py run "Explain this function"
  python3 code-review-opencode.py run-file prompt.txt -o output.txt
  python3 code-review-opencode.py review src/auth/ --plan --output auth-plan
  python3 code-review-opencode.py review main.py --focus "security,performance"
  python3 code-review-opencode.py import .claude/plans/review-src.md
""",
    )
    ...
    # check command
    check_parser = subparsers.add_parser("check", help="Validate OpenCode CLI availability")
    check_parser.add_argument("-v", "--verbose", action="store_true", help="Show version info")

    # run command
    run_parser = subparsers.add_parser("run", help="Run a short prompt via OpenCode CLI")
    run_parser.add_argument("prompt", help="The prompt to send to OpenCode")
