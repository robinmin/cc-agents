...
async function fetchUserPosts(userId: string): Promise<Post[]> {
    // Simulate API call
    return [{ id: '1', userId, title: 'First Post', content: 'Hello World' }];
}

async function fetchPostComments(postId: string): Promise<Comment[]> {
    // Simulate API call
    return [{ id: 'c1', postId, text: 'Great post!' }];
}
...
type PipelineResult<T, E = Error> = { success: true; data: T } | { success: false; error: E };

async function fetchWithErrorHandling<T>(fn: () => Promise<T>): Promise<PipelineResult<T>> {
    try {
        const data = await fn();
...
async function getUserDataSafe(userId: string) {
    ...
    const postsResult = await fetchWithErrorHandling(() => fetchUserPosts(userResult.data.id));
...
async function processBatch<T, R>(items: T[], processor: (item: T) => Promise<R>, batchSize = 5): Promise<R[]> {
    const results: R[] = [];
...
async function orchestrateUserData(userId: string): Promise<EnrichedUserData> {
    ...
    const commentsArray = await Promise.all(posts.map((post) => fetchPostComments(post.id)));
