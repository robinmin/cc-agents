#!/usr/bin/env python3
"""
Claude Code Review Utility - CLI tool for Claude Code-native code review and planning.

Usage:
    python3 code-review-claude.py <command> [options]
...
Examples:
    python3 code-review-claude.py check
    python3 code-review-claude.py run "Explain this function"
    python3 code-review-claude.py run-file prompt.txt
    python3 code-review-claude.py review src/auth/ --output plan.md
"""
...
Claude Code CLI is required for code review operations.
...
After installation, ensure 'claude' is available in your PATH.""",
    ...
    try:
        ...
        if result.returncode == 0:
            ...
            # Even if --version fails, if claude exists, consider it available
            return CheckResult(
                available=True,
                message="claude ready (version unknown)",
                version=None,
            )
        ...
        # Even if there's an exception, if claude exists, consider it available
        return CheckResult(
            available=True,
            message="claude ready",
            version=None,
...
def run_claude_prompt(
    ...
    """
    Run a short prompt via Claude CLI.
...
def cmd_run(args: argparse.Namespace) -> int:
    ...
    check_result = check_claude_availability()
...
def run_claude_from_file(
    prompt_file: Path,
    timeout: int = TIMEOUT_MODERATE,
    output_file: Path | None = None,
) -> RunResult:
    """
    Run a long prompt from a file via Claude CLI.
...
def extract_review_sections(content: str) -> dict[str, str]:
    """
    Extract structured sections from Claude's review output.

    Args:
        content: Raw review output from Claude.
...
def cmd_review(args: argparse.Namespace) -> int:
    ...
    prompt_file = generate_temp_path("claude-prompt")
    ...
    print(f"Running Claude Code ({mode} mode)...", file=sys.stderr)
...
def main() -> int:
    ...
    parser = argparse.ArgumentParser(
        description="Claude Code Review Utility - CLI tool for Claude Code-native code review and planning.",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Commands:
  check       Validate Claude CLI availability
  run         Run a short prompt via Claude CLI
...
""",
    ...
    # check command
    check_parser = subparsers.add_parser("check", help="Validate Claude CLI availability")
