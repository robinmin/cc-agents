#!/usr/bin/env python3
"""
Gemini Code Review Utility - CLI tool for Gemini-powered code review and planning.

Usage:
    python3 code-review-gemini.py <command> [options]
...
Examples:
    ...
    python3 code-review-gemini.py run "Explain this function"
    python3 code-review-gemini.py run-file prompt.txt --model gemini-2.5-flash
    python3 code-review-gemini.py review src/auth/ --output plan.md
"""
...
def generate_temp_path(prefix: str = "gemini") -> Path:
...
def cmd_run(args: argparse.Namespace) -> int:
    ...
    check_result = check_gemini_availability()
    if not check_result.available:
        print(check_result.message, file=sys.stderr)
        return 1

    # Determine timeout based on complexity hint
    ...
    if result.success:
        ...
        if hasattr(args, "save") and args.save:
            save_to_plan(result.output, args.save, result.model)
...
def run_gemini_from_file(
    ...
    Uses the temp file approach for long prompts:
    1. Read prompt from file
    2. Execute Gemini with prompt content
    3. Optionally save output to file
...
def extract_review_sections(content: str) -> dict[str, str]:
    """
    Extract structured sections from Gemini's review output.

    Args:
        content: Raw review output from Gemini.
...
def save_to_plan(content: str, plan_name: str, model: str | None = None) -> Path:
    """
...
def cmd_review(args: argparse.Namespace) -> int:
    ...
    prompt_file = generate_temp_path("gemini-prompt")
    ...
    print(f"Running Gemini {args.model} ({mode} mode)...", file=sys.stderr)
...
class ReviewIssue(NamedTuple):
    ...
    identifier: str  # e.g., CRITICAL-001, or auto-generated
    title: str
...
def extract_issues_from_section(section_content: str, priority: str) -> list[ReviewIssue]:
    ...
    for line in lines:
        ...
        # Detect structured issue format: **[CRITICAL-001]** or **[ISSUE-001]**
...
def create_task_from_issue(issue: ReviewIssue, review_metadata: dict[str, str]) -> tuple[bool, str]:
    ...
    """
    # Generate task name from identifier and title
    ...
    if review_metadata.get("model"):
        background_parts.append(f"**Reviewed by**: {review_metadata['model']}")
    ...
    try:
        ...
        # Task file should be created in docs/prompts/
        ...
        # Read and update the task file
        task_content = task_file.read_text()

        # Update Background and Requirements sections
        # Tasks typically have a structure with ---\n# Background\n# Requirements\n# Solutions\n# References
        # Use regex to properly insert content after section headers
        import re

        # Handle both # Background and ## Background
...
def main() -> int:
    ...
    parser = argparse.ArgumentParser(
        description="Gemini Code Review Utility - CLI tool for Gemini-powered code review and planning.",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Commands:
  check       Validate Gemini CLI availability
  run         Run a short prompt via Gemini CLI
  run-file    Run a long prompt from a file
  review      Comprehensive code review

Examples:
  python3 code-review-gemini.py check
  python3 code-review-gemini.py run "Explain this function" --model gemini-2.5-flash
  python3 code-review-gemini.py run-file prompt.txt -o output.txt
  python3 code-review-gemini.py review src/auth/ --plan --output auth-plan
  python3 code-review-gemini.py review main.py --focus "security,performance"

Available Models:
  gemini-2.5-pro        Complex analysis, comprehensive review (default)
  gemini-2.5-flash      Quick feedback, single-file review
  gemini-2.5-flash-lite Fastest, cost-efficient
  gemini-3-pro-preview  Latest, most capable (preview)
  gemini-3-flash-preview Latest, balanced speed/capability (preview)
""",
    )
    ...
    # check command
    check_parser = subparsers.add_parser("check", help="Validate Gemini CLI availability")
