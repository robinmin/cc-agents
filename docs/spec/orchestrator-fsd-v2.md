...
|------|------|---------|-------------|
| `--state-dir <path>` | string | `docs/.workflow-runs` | State directory. Env: `ORCHESTRATOR_STATE_DIR` |
...
5. Gate type must be `command`, `auto`, or `human`
...
phases:                              # Required, DAG nodes
  <phase-name>:                      # Lowercase alphanumeric + hyphens
    ...
    gate:                            # Optional
      type: command | auto | human  # Gate type
      rework:                        # Optional
...
| Gate `type` | Must be `command`, `auto`, or `human` | Error |
...
*End of Functional Specification. v1.1.0 — Lord Robb — 2026-04-09.*
