...
16. [Developer Slash Commands](#16-developer-slash-commands)
...
## 16. Developer Slash Commands

The `dev-*` slash command family provides Claude Code-native shortcuts for the 9-phase pipeline. These commands live in `plugins/rd3/commands/` and come in two flavors.

### 16.1 Command Overview

| Command | Type | Description |
|---------|------|-------------|
| `/rd3:dev-run` | Orchestration | Profile-driven pipeline execution (primary entry point) |
| `/rd3:dev-refine` | Orchestration | Phase 1 refine — structured quality analysis |
| `/rd3:dev-plan` | Orchestration | Phases 2–4 — architecture, design, decomposition |
| `/rd3:dev-unit` | Orchestration | Phase 6 unit test — generate tests to reach coverage target |
| `/rd3:dev-review` | Orchestration | Phase 7 code review — task-scoped quality gate |
| `/rd3:dev-docs` | Orchestration | Phase 9 docs — canonical doc refresh |
| `/rd3:dev-gitmsg` | Standalone | Generate conventional commit message from staged changes |
| `/rd3:dev-changelog` | Standalone | Generate changelog from git commits |
| `/rd3:dev-fixall` | Standalone | Systematically fix all lint, type, and test errors |
...
**Canonical doc refresh.** `dev-docs` refreshes the four canonical cumulative docs (`01_ARCHITECTURE_SPEC.md`, `02_DEVELOPER_SPEC.md`, `03_USER_MANUAL.md`, `99_EXPERIENCE.md`) rather than generating API artifacts.

### 16.3 Invocation Patterns

#### Claude Code (native)

```bash
/rd3:dev-run 0274 --preset complex
...
```bash
orchestrator run 0274 --preset complex
orchestrator run 0274 --preset refine
bun run scripts/dev-fixall.ts
```
...
```mermaid
flowchart TD
    subgraph "Orchestration-Backed"
        A["/rd3:dev-run 0274"] --> B[Orchestrator: DAG + FSM]
        B --> C[Phases 1-9 via rd3 skills]
    end
    subgraph "Standalone Utilities"
        D["/rd3:dev-gitmsg"] --> E[git analysis]
        F["/rd3:dev-changelog"] --> G[commit parsing]
        H["/rd3:dev-fixall"] --> I[validation loop]
    end
```

### 16.5 Standalone vs Orchestration-Backed Decision

| Criterion | Orchestration-Backed | Standalone |
|-----------|---------------------|------------|
| Phase in pipeline | ✅ Yes (phases 1–9) | ❌ No |
| Benefits from FSM/DAG | ✅ Rework, gates, state | ❌ No |
| Can run on remote channel | ✅ Via ACP | ❌ Local only |
| Thin wrapper overhead | Low (delegation) | None |

Use orchestration-backed for anything tied to task lifecycle, quality gates, or cross-channel execution. Use standalone for git metadata and validation repair — workflows where orchestration adds no value.

### 16.6 Channel Contract
