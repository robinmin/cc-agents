...
| Module | Key Exports |
|--------|-------------|
| `lib/config.ts` | `getProjectRoot()`, `loadConfig()`, `saveConfig()`, `resolveActiveFolder()` |
| `lib/taskFile.ts` | `readTaskFile()`, `parseFrontmatter()`, `updateStatus()`, `updateSection()`, `validateTaskForTransition()` |
| `lib/wbs.ts` | `getNextWbs()`, `formatWbs()`, `findTaskByWbs()` |
| `lib/kanban.ts` | `refreshKanban()`, `renderKanban()`, `buildKanbanFromFolder()` |
| `lib/template.ts` | `applyTemplate()`, `loadTemplate()` |
| `server/router.ts` | `createRouter()`, `registerHandlers()` |
| `server/sse.ts` | `SSEManager` — `broadcast()`, `addClient()`, `removeClient()` |
| `server/writeLock.ts` | `acquireLock()`, `releaseLock()` |

---

*End of Architecture Document. v2.1.0 — Generated by Antigravity — 2026-04-04.*
