...
| `DELETE` | `/tasks/:wbs` | Delete a task (sets status to Canceled) |
| `POST` | `/tasks/:wbs/artifacts` | Upload artifact to task (multipart/form-data) |
| `GET` | `/tasks/:wbs/artifacts` | List artifacts for a task (supports `?artifactType=`) |
| `GET` | `/tasks/:wbs/tree` | Get artifact tree for a task |
| `GET` | `/tasks/:wbs/check` | Run validation on a task |
| `POST` | `/tasks/:wbs/actions` | Execute task actions via orchestrator (refine, plan, run, verify, decompose, evaluate) |
| `POST` | `/tasks/batch-create` | Batch create multiple tasks (JSON `{ items: [...] }`) |
| `POST` | `/tasks/refresh` | Refresh kanban boards |
| `GET` | `/config` | Get server configuration |
| `PATCH` | `/config` | Update server configuration |
| `GET` | `/config/template` | Get task creation template |
| `GET` | `/events` | SSE stream for real-time updates (supports `?status=<status>` filter) |
...
eventSource.addEventListener('created', (e) => {
  const { wbs, status } = JSON.parse(e.data);
  // Add to kanban board
});
...
Event types: `created`, `updated`, `deleted`, `config.updated`, `refresh.completed`.
...
*End of Functional Specification Document. v2.4.0 — Generated by Antigravity — 2026-04-14.*
